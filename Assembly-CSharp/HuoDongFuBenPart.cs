...
public class HuoDongFuBenPart : UserControl
{
	public HuoDongFuBenPart()
	{
	...
	private void InitActivityList()
	{
		...
		for (int i = 0; i < xelementList.Count; i++)
		{
			...
			if (Global.GetXElementAttributeInt(xelement, "Type") == 2)
			{
				...
				dailyTasksItem.MaxEnterNum = (double)Global.GetXElementAttributeInt(xelement, "Information");
				dailyTasksItem.Jindu = StringUtil.substitute(Global.GetLang("完成进度：{0}/{1}"), new object[]
				{
					num2,
					(!string.IsNullOrEmpty(Global.GetXElementAttributeStr(xelement, "Information"))) ? Global.GetXElementAttributeStr(xelement, "Information") : Global.GetLang("无限")
				});
	...
	{
		this.CanShowItemsList = new List<DailyTasksItem>();
		if (this.ItemsList != null)
		{
			for (int i = 0; i < this.ItemsList.Count; i++)
			{
				this.CanShowItemsList.Add(this.ItemsList[i]);
			}
		}
	}
	...
	{
		this.FilterItemList();
		if (this.CanShowItemsList == null)
		{
			return;
		}
		for (int i = 0; i < this.CanShowItemsList.Count; i++)
		{
			FuBenData fuBenData = Global.GetFuBenData(this.CanShowItemsList[i].ItemTag);
			if (fuBenData != null)
			{
				this.CanShowItemsList[i].Jindu = StringUtil.substitute(Global.GetLang("完成进度：{0}/{1}"), new object[]
				{
					fuBenData.EnterNum,
					(this.CanShowItemsList[i].MaxEnterNum > 0.0) ? this.CanShowItemsList[i].MaxEnterNum.ToString() : Global.GetLang("无限")
				});
			}
		}
	}
	...
	private List<DailyTasksItem> CanShowItemsList = new List<DailyTasksItem>();
