...
public class QiFuGetGiftsPart : UserControl
{
	...
	protected override void InitializeComponent()
	{
		...
		this.btnNext.MouseLeftButtonUp = delegate(object s, MouseEvent e)
		{
			...
			{
				int goodsID = int.Parse(ConfigSystemParam.GetSystemParamByName("JinDan", true));
				int totalGoodsCountByID = Global.GetTotalGoodsCountByID(goodsID);
				if (200 > Global.GetRoleOwnNumByMoneyType(163) && totalGoodsCountByID <= 0 && !IConfigbase<ConfigDaiBiShiYong>.Instance.SendHuoBiNumber("QiFuXiTong", 200, true))
				{
				...
				if (totalGoodsCountByID <= 0 && Global.GetZuanShi(ZuanShiPartClass.QiFu))
				{
			...
			else
			{
				...
				if (Global.GetZuanShi(ZuanShiPartClass.QiFu))
				{
					string text2 = IConfigbase<ConfigDaiBiShiYong>.Instance.RefreshString(Global.GetLang("幸运之星"), "QiFuXiTong", 1800);
					GChildWindow messageBoxWindow = Super.ShowMessageBox(Super.MainWindowRoot, 2, Global.GetLang("提示"), Global.GetColorStringForNGUIText(new object[]
					{
						"e3b36c",
						string.Format(Global.GetLang("需要消耗{0}{1}，确定吗？"), 1800, text2)
					}), -1, -1, -1, -1, 0.7, default(Vector3), null, null);
					if (messageBoxWindow.GetComponentInChildren<MyMessageBoxPart>() != null)
					{
						messageBoxWindow.GetComponentInChildren<MyMessageBoxPart>().CheckBox.isChecked = Global.ZuanShiIsCheck;
	...
	public void RefreshUIUnit(bool oneTime)
	{
		...
		if (oneTime)
		{
			...
			{
				IConfigbase<ConfigDaiBiShiYong>.Instance.RefreshShowImg(this.TextureUnit, "QiFuXiTong", 200, "NetImages/GameRes/Images/Unit/xingyunzhixing.png");
				this.TextUnit.text = "200";
				this.ButtonNextLabel.text = Global.GetLang("再抽一次");
			}
		}
		else
		{
			IConfigbase<ConfigDaiBiShiYong>.Instance.RefreshShowImg(this.TextureUnit, "QiFuXiTong", 1800, "NetImages/GameRes/Images/Unit/xingyunzhixing.png");
			this.TextUnit.text = "1800";
