...
public class MUJieriPartmeiriChongzhiItem : UserControl
{
	...
	protected override void InitializeComponent()
	{
		...
		{
			if (this.m_AwardGiftGainState == JieriAwardGiftGainState.CanNotGain)
			{
				PlayZone.GlobalPlayZone.ShowChongZhiWindow();
			}
			else if (this.m_AwardGiftGainState == JieriAwardGiftGainState.CanGain)
			{
				GameInstance.Game.SpriteFetchActivityAward(12, this.Id);
			}
		};
	...
	{
		this.m_Btnlabel.text = Global.GetLang("充 值");
	}
	...
	public int Need
	{
		...
		set
		{
			...
			{
				"dac7ae",
				string.Format(Global.GetLang("今日充值达{0}钻石"), this.Need)
			});
		}
	}
	...
	public JieriAwardGiftGainState AwardGiftGainState
	{
		...
		set
		{
			...
			case JieriAwardGiftGainState.CanGain:
				...
				this.ChangeBtnState(this.btnChongZhi, "btn_green_normal ", "btn_green_hover");
				this.m_Btnlabel.text = Global.GetLang("领 取");
			...
			case JieriAwardGiftGainState.Gained:
				...
				this.btnChongZhi.gameObject.SetActive(false);
				this.m_Btnlabel.text = string.Empty;
			...
			case JieriAwardGiftGainState.CanNotGain:
				...
				this.stateSpriteGained.gameObject.SetActive(false);
				this.btnChongZhi.gameObject.SetActive(true);
				this.ChangeBtnState(this.btnChongZhi, "btn_chongzhi_normal", "btn_chongzhi_hover");
				this.m_Btnlabel.text = Global.GetLang("充 值");
				this.labNeed.gameObject.SetActive(true);
				break;
			}
	...
	private void ChangeBtnState(GButton btn, string Normal, string Other)
	{
		btn.normalSprite = Normal;
		btn.hoverSprite = Other;
		btn.pressedSprite = Other;
		btn.disabledSprite = Other;
		btn.Refresh();
	}
