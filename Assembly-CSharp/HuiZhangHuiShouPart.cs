...
public class HuiZhangHuiShouPart : UserControl
{
	...
	public void DragDrop(object sender, object e)
	{
		if (this.list != Super.GData.DragingListBox && null != Super.GData.DragingItem && Super.GData.DragingItem.BoxTypes == 1)
		{
			...
			if (this.gd.Using <= 0)
			{
				...
				int lieShaPrice = goodsXmlNodeByID.LieShaPrice;
				if (lieShaPrice <= 0)
				{
					GGameInfocs.AddGameInfoMessage(GameInfoTypeIndexes.Error, ShowGameInfoTypes.ErrAndBox, StringUtil.substitute(Global.GetLang("该装备无法兑换到猎杀值"), new object[0]), 0, -1, -1, 0);
					return;
				}
				...
				long systemParamIntByName = ConfigSystemParam.GetSystemParamIntByName("HuiZhangExchange");
				this.lieShaZhiTitle.Text = Global.GetLang("可兑换值:");
				this.lieShaZhiText.Text = StringUtil.substitute("{0}-{1}", new object[]
				{
					(int)((double)((long)lieShaPrice * systemParamIntByName) / 100.0),
					lieShaPrice
				});
			}
		}
	...
	private void AddGoodsIcon(int goodsID, int gcount, int quality, int forgeLevel, int addPropIndex, int bornIndex, int binding, bool clear = false, int Id = -1)
	{
		...
		if (goodsXmlNodeByID != null)
		{
			...
			this.LieShaPrice = goodsXmlNodeByID.LieShaPrice;
		}
	}
	...
	private void ZhuangBeiHuiShou()
	{
		...
		GameInstance.Game.SpriteHuiZhangExchange(this.gd.Id);
	}
	...
	public void OnExchangeEnd(int result, int goodsDbID, int lieShaValue)
	{
		if (result < 1)
		{
			GGameInfocs.AddGameInfoMessage(GameInfoTypeIndexes.Error, ShowGameInfoTypes.ErrAndBox, StringUtil.substitute(Global.GetLang("徽章兑换时发生错误:{0}"), new object[]
			{
				result
			}), 0, -1, -1, 0);
		}
		else
		{
			...
			this.gd = null;
			GGameInfocs.AddGameInfoMessage(GameInfoTypeIndexes.Error, ShowGameInfoTypes.ErrAndBox, StringUtil.substitute(Global.GetLang("徽章置换成功，得到{0}猎杀值"), new object[]
			{
				lieShaValue
			}), 0, -1, -1, 0);
		}
