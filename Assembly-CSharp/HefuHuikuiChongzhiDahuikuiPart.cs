...
public class HefuHuikuiChongzhiDahuikuiPart : UserControl
{
	...
	public void InitPartSize(int width, int height)
	{
		...
		gicon.MouseLeftButtonUp = delegate(object s, MouseEvent e)
		{
			...
			GGameInfocs.AddGameInfoMessage(GameInfoTypeIndexes.Hot, ShowGameInfoTypes.ErrAndBox, Global.GetLang("已经过了活动时间，无法领取"), 0, -1, -1, 0);
		};
		this.BtnArr[0] = gicon;
	}
	...
	public void GetData(bool isInLingquTime)
	{
		...
		if (this.state < 0)
		{
			this.LoadingWin = U3DUtils.NEW<LoadingWindow>();
			this.Container.Children.Add(this.LoadingWin);
			GameInstance.Game.SpriteQueryHeFuCZSong();
		}
		else
		{
			this.SetBtnState(0, this.state);
		}
	}

	public void OnGetDataCompleted(int result, int minYuanBao, int roleYuanBaoInPeriod)
	{
	...
	public void OnLingquCompleted(int result)
	{
		...
		if (result >= 0)
		{
			GGameInfocs.AddGameInfoMessage(GameInfoTypeIndexes.Hot, ShowGameInfoTypes.ErrAndBox, Global.GetLang("恭喜你，成功领取了合服大礼包"), 0, -1, -1, 0);
			this.SetBtnState(0, result);
			this.state = result;
		}
	}
