...
public class HuodongPartChongzhiHuikuiPartLeijiChongzhiPart : UserControl
{
	...
	protected override void InitializeComponent()
	{
		...
		GameInstance.Game.QueryPayActiveInfo(Global.Data.roleData.RoleID, 38);
	...
	private void showList(List<XElement> xmlList)
	{
		...
		for (int i = 0; i < xmlList.Count; i++)
		{
			...
			if (this.sumDepositDiamonds < xelementAttributeInt)
			{
				break;
			}
			...
			item.TxtValue.Text = StringUtil.substitute(Global.GetLang("累计充值{0}钻石"), new object[]
			{
				Global.GetXElementAttributeStr(xelement, "MinYuanBao")
			});
			item.TxtValue.gameObject.SetActive(false);
			item.m_LblShowText.text = StringUtil.substitute(Global.GetLang("累计充值{0}钻石"), new object[]
			{
				Global.GetXElementAttributeStr(xelement, "MinYuanBao")
			});
			...
			item.SubmitBtn.MouseLeftButtonUp = delegate(object s, MouseEvent e)
			{
				...
				GameInstance.Game.GetChongZhiJiangLi(Global.Data.roleData.RoleID, 38, Convert.ToInt32(item.SubmitBtn.BtnTag));
			};
			if (this.isLingqu(i))
			{
				item.LingquFlag = 0;
