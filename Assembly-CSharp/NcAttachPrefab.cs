...
public class NcAttachPrefab : NcEffectBehaviour
{
	...
	private void CreateAttachPrefab()
	{
		...
		this.m_CreateGameObject.transform.position = new Vector3(Random.Range(-this.m_RandomRange.x, this.m_RandomRange.x) + position.x, Random.Range(-this.m_RandomRange.y, this.m_RandomRange.y) + position.y, Random.Range(-this.m_RandomRange.z, this.m_RandomRange.z) + position.z);
		...
		GameObject createGameObject = this.m_CreateGameObject;
		createGameObject.name = createGameObject.name + " " + this.m_nCreateCount;
