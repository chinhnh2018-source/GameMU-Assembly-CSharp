...
public class JieriDengluPart : UserControl
{
	public JieriDengluPart()
	{
	...
	private void LoadDengluDaliJiangLi(List<XElement> xml)
	{
		...
		int num = (this.state != null) ? this.state.LimitTimeLoginNum : 0;
		...
		for (int i = 0; i < xml.Count; i++)
		{
			...
			gicon.MouseLeftButtonUp = delegate(object s, MouseEvent e)
			{
				...
				if (this.IsInLingquTime)
				{
					...
					GameInstance.Game.SpriteFetchActivityAward(10, (s as GIcon).ItemCode);
					return;
				}
			...
			jieriDengluDaliJiangliListItem.DayNum = StringUtil.substitute(Global.GetLang("累计登陆{0}天"), new object[]
			{
	...
	{
		int resource = (this.state != null) ? this.state.LimitTimeGiftState : 0;
	...
	public void OnGetDataCompleted(HuodongData result)
	{
	...
	public void OnLingquCompleted(int result)
	{
		...
		if (result >= 0)
		{
			...
			this.state.LimitTimeGiftState = result;
			this.LoadDengluDaliJiangLi(this.xmlList);
		}
	}
