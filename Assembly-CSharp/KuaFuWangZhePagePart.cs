...
public class KuaFuWangZhePagePart : UserControl
{
	...
	public void RefreshOnsaleInfo(int xmlID, int finishNum)
	{
		for (int i = 0; i < this.listBox.Count(); i++)
		{
			...
			{
				mallItem.nXianGouHaveBuy = finishNum;
				mallItem.wangzheXianGouLabel.text = mallItem.nXianGouLeft.ToString();
			}
		}
	}
	...
	{
		MallItem mi = U3DUtils.AS<MallItem>(this.listBox.SelectedItem);
		this.showSelectTip(mi, e);
	}
	...
	private void StartGouMai(int buyGoodsID, int itemID, int num, bool isXianGou = false)
	{
		...
		KuaFuWangZheMallPart.Send_CMD_SPR_KINGOFBATTLE_MALL_BUY(itemID, num);
	}
	...
	private IEnumerator LoadMallItem(int StartIdx)
	{
		...
		for (int i = StartIdx; i < limitPerFrame; i++)
		{
			...
			if (KuaFuWangZheMallPart.ServerSaleDataOf(mallID) != null)
			{
				int WangZheJiFen = Global.GetXElementAttributeInt(this._mallItemXmlList[i], "WangZheJiFen");
				int SinglePurchase = Global.GetXElementAttributeInt(this._mallItemXmlList[i], "SinglePurchase");
				int BeginNum = Global.GetXElementAttributeInt(this._mallItemXmlList[i], "BeginNum");
				int EndNum = Global.GetXElementAttributeInt(this._mallItemXmlList[i], "EndNum");
				...
				int nXianGouHaveBuy = 0;
				KingOfBattleStoreSaleData saleData = KuaFuWangZheMallPart.ServerSaleDataOf(mallID);
				if (saleData != null)
				{
					nXianGouHaveBuy = saleData.Purchase;
				}
				...
				si.GoodsPrice = WangZheJiFen.ToString();
				si.ItemDiamondIcon.spriteName = "wangzhejf";
				si.TabPageID = this.tabID;
				si.ItemType = 0;
				si.goodsOwnerTypes = GoodsOwnerTypes.WangZheShangCheng;
				si.CreateGGoodsIcon(goods);
				si.nXianGouMax = SinglePurchase;
				si.nXianGouHaveBuy = nXianGouHaveBuy;
				si.wangzheXianGou.gameObject.SetActive(true);
				si.wangzheXianGouLabel.text = si.nXianGouLeft.ToString();
				si.MouseLeftButtonUp = delegate(object s, MouseEvent e)
				{
					MallItem mi = s as MallItem;
					this.showSelectTip(mi, e);
				};
				si.ItemIcon.DPSelectedItem = delegate(object s1, DPSelectedItemEventArgs e1)
				{
					...
					if (e1.IDType == 8)
					{
						if (KuaFuWangZheMallPart.JiFen < int.Parse(mallItem.GoodsPrice))
						{
							Super.HintMainText(Global.GetLang("您当前拥有的王者点数不足，无法进行购买!"), 10, 3);
							return;
						}
