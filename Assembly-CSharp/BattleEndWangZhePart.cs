...
public class BattleEndWangZhePart : UserControl
{
	...
	protected void Close(object sender, MouseEvent e)
	{
		...
		{
			TCPGameServerCmds.CMD_SPR_KINGOFBATTLE_AWARD_GET.SendDataUseRoleID();
		}
		this.CloseWindow();
	}

	public void InitPartData(string Title, bool success, int score, long expr, int chengjiu, int JiaoTuanScore, int mengJunScore, int moJing, int paiMing, string awardsGoods, int JinBi, KingOfBattleAwardsData result)
	{
		...
		if (result != null)
		{
			this.SetMVP(result.MvpRoleName, result.MvpOccupation, result.MvpRoleSex);
		}
	}
	...
	{
		if (PlayZone.GlobalPlayZone._BattleEndWangZheWindow != null)
		{
			this.AnimWin.gameObject.SetActive(false);
			this.AnimLose.gameObject.SetActive(false);
			NGUITools.DestroyImmediate(this.AnimWin.gameObject);
			NGUITools.DestroyImmediate(this.AnimLose.gameObject);
			Super.CloseChildWindow(PlayZone.GlobalPlayZone.Children, PlayZone.GlobalPlayZone._BattleEndWangZheWindow);
			PlayZone.GlobalPlayZone._BattleEndWangZheWindow = null;
			PlayZone.GlobalPlayZone._BattleEndWangZhe = null;
		}
	}
