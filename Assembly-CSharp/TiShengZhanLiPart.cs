...
public class TiShengZhanLiPart : UserControl
{
	protected override void InitializeComponent()
	{
		...
		this.TabFileList.Add(Global.GetLang("急需提升"));
		this.TabFileList.Add(Global.GetLang("有待提高"));
		this.TabFileList.Add(Global.GetLang("完美无瑕"));
		this.InitBtnProc();
		this.m_GiftItemOBC = this.m_ListGiftItems.ItemsSource;
		Global.StartRequest();
		this.requstRetainCount = Global.requstRetainCount;
	}
	...
	{
		this.m_PnlGift.transform.localPosition = new Vector3(84f, 85f, -0.5f);
		UIPanel component = this.m_PnlGift.GetComponent<UIPanel>();
		if (component != null)
		{
			component.clipRange = new Vector4(0f, -143f, 780f, 384f);
		}
	...
	private void SetTabBtnIndex(int nIndex)
	{
		...
		at = this.m_TabBtnOBC.GetAt(0);
		component = at.GetComponent<BtnObjForBianQiang>();
		if (nIndex == 0)
		{
			component.m_BtnItem.Label.color = NGUIMath.HexToColorEx(16711680U);
		}
		else
		{
			component.m_BtnItem.Label.color = NGUIMath.HexToColorEx(10551296U);
		}
		at = this.m_TabBtnOBC.GetAt(1);
		component = at.GetComponent<BtnObjForBianQiang>();
		if (nIndex == 1)
		{
			component.m_BtnItem.Label.color = NGUIMath.HexToColorEx(15849216U);
		}
		else
		{
			component.m_BtnItem.Label.color = NGUIMath.HexToColorEx(8806913U);
		}
		at = this.m_TabBtnOBC.GetAt(2);
		component = at.GetComponent<BtnObjForBianQiang>();
		if (nIndex == 2)
		{
			component.m_BtnItem.Label.color = NGUIMath.HexToColorEx(851712U);
		}
		else
		{
			component.m_BtnItem.Label.color = NGUIMath.HexToColorEx(357120U);
		}
		List<TiShengZhanLiItemVO> dataList = this.GetDataList(nIndex);
		this.RefrshByData(dataList);
	}
	...
	private List<TiShengZhanLiItemVO> GetDataList(int index)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			else if (xelementAttributeInt2 == 6)
			{
				int xelementAttributeInt4 = Global.GetXElementAttributeInt(xelement, "Parameter");
				if (Global.Data.roleData.RebornLevel >= xelementAttributeInt4)
				{
					TiShengZhanLiItemVO itemVO6 = this.GetItemVO(xelementAttributeInt, xelement);
					if (itemVO6 != null)
					{
						if (index == 0)
						{
							if (itemVO6.percent <= 0.5f)
							{
								list.Add(itemVO6);
							}
						}
						...
						{
							if (itemVO6.percent > 0.5f && itemVO6.percent < 1f)
							{
								list.Add(itemVO6);
							}
						}
						else if (index == 2 && itemVO6.percent == 1f)
						{
							list.Add(itemVO6);
						}
					}
	...
	private TiShengZhanLiItemVO GetItemVO(int ID, XElement item)
	{
		...
		else if (tiShengZhanLiItemVO.ItemType == 32)
		{
			float num44 = 0f;
			for (int i = 1; i < 11; i++)
			{
				RebornEquipData levelByEquipKey = IConfigbase<ConfigChongShengEquipCuiLian>.Instance.GetLevelByEquipKey((EquipCuiLian)i, Global.Data.RoleID);
				if (levelByEquipKey != null)
				{
					num44 += (float)levelByEquipKey.Level;
				}
			}
			float num45 = num44 / (float)tiShengZhanLiItemVO.StandardValue;
			num = num45;
		}
		else if (tiShengZhanLiItemVO.ItemType == 33)
		{
			int stageLevel = MoShenMiBaoData.GetStageLevel(MoShenMiBaoType.MoShenJiao);
			int stageLevel2 = MoShenMiBaoData.GetStageLevel(MoShenMiBaoType.MoShenYan);
			int stageLevel3 = MoShenMiBaoData.GetStageLevel(MoShenMiBaoType.MoShenYu);
			float num46 = (float)(stageLevel + stageLevel2 + stageLevel3);
			float num47 = num46 / (float)tiShengZhanLiItemVO.StandardValue;
			num = num47;
		}
		if (num < 0f)
		{
	...
	public void RefrshByData(List<TiShengZhanLiItemVO> dataList)
	{
		...
		if (dataList != null)
		{
			...
			for (int i = 0; i < count; i++)
			{
				TiShengZhanLiPartItem tiShengZhanLiPartItem = U3DUtils.NEW<TiShengZhanLiPartItem>();
				tiShengZhanLiPartItem.DPSelectedItem = delegate(object s, DPSelectedItemEventArgs e)
				{
				...
				TiShengZhanLiItemVO tiShengZhanLiItemVO = dataList[i];
				tiShengZhanLiPartItem.RefreshUI(tiShengZhanLiItemVO.percent, tiShengZhanLiItemVO.StandardValue, tiShengZhanLiItemVO.Name, tiShengZhanLiItemVO.LinkID, tiShengZhanLiItemVO.ImageName, tiShengZhanLiItemVO.ItemType);
				this.m_GiftItemOBC.AddNoUpdate(tiShengZhanLiPartItem);
			}
		}
		if (this.m_GiftItemOBC.Count == 0)
		{
			this.m_SpriteZhanWu.gameObject.SetActive(true);
		}
		else
		{
			this.m_SpriteZhanWu.gameObject.SetActive(false);
		}
	}
	...
	public void OnRequestCallBack_QueryStar(Dictionary<int, int> dic)
	{
		if (dic != null)
		{
			...
			TiShengZhanLiPart.OPNE_XIN_HUN_COUNT = 0;
			foreach (KeyValuePair<int, int> keyValuePair in this.m_DicActiveStar)
			{
				TiShengZhanLiPart.OPNE_XIN_HUN_COUNT += keyValuePair.Value;
			}
		}
		this.OnRequestCallBack_CheckPending();
	}
	...
	private void RefreshImprovingView()
	{
	...
	public void RecvZuoQiLevel(int level)
	{
		Global.zuoQiLevelByBianQiang = level;
		this.OnRequestCallBack_CheckPending();
	}
	...
	public UISprite m_SpriteZhanWu;
