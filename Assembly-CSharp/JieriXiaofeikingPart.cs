...
public class JieriXiaofeikingPart : UserControl
{
	...
	public void InitPartSize(int width, int height)
	{
		...
		gicon.MouseLeftButtonUp = delegate(object s, MouseEvent e)
		{
			...
			if (this.IsInLingquTime)
			{
				...
				GameInstance.Game.SpriteFetchActivityAward(15, 0);
				return;
			}
	...
	private void LoadDengluDaliJiangLi(List<XElement> xml)
	{
		...
		for (int i = 0; i < xml.Count; i++)
		{
			...
			jieriKingListItem.txtCondition.text = StringUtil.substitute(Global.GetLang("最低消费 {0} 钻石"), new object[]
			{
	...
	public void OnLingquCompleted(int result)
	{
		...
		if (result >= 0)
		{
			GGameInfocs.AddGameInfoMessage(GameInfoTypeIndexes.Hot, ShowGameInfoTypes.ErrAndBox, Global.GetLang("恭喜你，成功领取了消费王大礼包"), 0, -1, -1, 0);
	...
	private string GetKingNameByPaiHang(int paiHang)
	{
		...
		for (int i = 0; i < listPaiHang.Count; i++)
		{
			...
			{
				result = listPaiHang[i].MaxLevelRoleName;
				break;
			}
		}
		return result;
