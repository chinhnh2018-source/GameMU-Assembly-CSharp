...
public class HefuPartLuolanZhengbaItem : UserControl
{
	private void InitTextInPrefabs()
	{
		this.lingquBtn.Text = Global.GetLang("领 取");
		this.labType.transform.localScale = new Vector3(20f, 20f, 1f);
		this.labText.X = 410.0;
	}
	...
	public int Id
	{
		...
		set
		{
			...
			if (this.Id == 1)
			{
				this.labType.text = Global.GetLang("罗兰城主奖励：");
				this.labText.text = Global.GetColorStringForNGUIText(new object[]
				{
					"ff0000",
					Global.GetLang("需要取得1次胜利")
				});
			}
			else if (this.Id == 2)
			{
				this.labType.text = Global.GetLang(string.Empty);
				this.labText.text = Global.GetColorStringForNGUIText(new object[]
				{
					"ff0000",
					Global.GetLang("需要取得2次胜利")
				});
			}
			else if (this.Id == 3)
			{
				this.labType.text = Global.GetLang("罗兰贵族奖励：");
				this.labText.text = Global.GetColorStringForNGUIText(new object[]
				{
					"ff0000",
					Global.GetLang("需要取得1次胜利")
	...
	public AwardGiftGainState state
	{
		get
		{
		...
		set
		{
			...
			case AwardGiftGainState.CanGain:
				...
				this.labText.gameObject.SetActive(false);
				break;
			...
			case AwardGiftGainState.CanNotGain:
				...
				this.stateSprite.gameObject.SetActive(false);
				this.lingquBtn.gameObject.SetActive(false);
				this.labText.gameObject.SetActive(true);
				break;
			}
		}
	}
	...
	public TextBlock labText;

	public DPSelectedItemEventHandler DPSelectedItem;

	private AwardGiftGainState m_GiftGainState = AwardGiftGainState.CanNotGain;
