...
public class MUJieripartChongzhiKing : UserControl
{
	...
	private void InitData(string strXML)
	{
		...
		for (int i = 0; i < xelementList3.Count; i++)
		{
			...
			MUJieripartChongzhiKingItem mujieripartChongzhiKingItem = U3DUtils.NEW<MUJieripartChongzhiKingItem>();
			this.ItemCollection.Add(mujieripartChongzhiKingItem);
			this.listItem.Add(mujieripartChongzhiKingItem);
			...
			mujieripartChongzhiKingItem.GoodsIDs = goodsIDs;
			mujieripartChongzhiKingItem.Id = xelementAttributeInt2;
			mujieripartChongzhiKingItem.Need = xelementAttributeInt3;
			mujieripartChongzhiKingItem.RoleName = this.GetKingNameByPaiHang(i + 1);
			mujieripartChongzhiKingItem.IsInLingqu = this.isInLingqu;
			UIPanel component = mujieripartChongzhiKingItem.GetComponent<UIPanel>();
			if (component)
			{
	...
	private bool IsMeByPaiHang()
	{
		...
		{
			if (this.listKingPaiHang[i] != null && Global.Data.UserID == this.listKingPaiHang[i].UserID)
			{
				result = true;
				this.rank = i;
			}
	...
	private void setState()
	{
		...
		MUJieripartChongzhiKingItem mujieripartChongzhiKingItem = this.listItem[paiHang - 1];
		mujieripartChongzhiKingItem.AwardGiftGainState = JieriAwardGiftGainState.CanNotGain;
		for (int i = 0; i < this.listItem.Count; i++)
		{
			if (flag && i == paiHang - 1)
			{
				...
				{
					mujieripartChongzhiKingItem = this.listItem[paiHang - 1];
					mujieripartChongzhiKingItem.AwardGiftGainState = JieriAwardGiftGainState.Gained;
				}
				if (this.state.State <= 0)
				{
					mujieripartChongzhiKingItem = this.listItem[paiHang - 1];
					mujieripartChongzhiKingItem.AwardGiftGainState = JieriAwardGiftGainState.CanGain;
				}
			}
			if (i != paiHang - 1)
			{
				mujieripartChongzhiKingItem = this.listItem[i];
				mujieripartChongzhiKingItem.AwardGiftGainState = JieriAwardGiftGainState.CanNotGain;
			}
		}
	}
	...
	private void setNullState()
	{
		for (int i = 0; i < this.listItem.Count; i++)
		{
			MUJieripartChongzhiKingItem mujieripartChongzhiKingItem = this.listItem[i];
			mujieripartChongzhiKingItem.AwardGiftGainState = JieriAwardGiftGainState.CanNotGain;
		}
	}
	...
	public void setCompletedInfo(int result, int position)
	{
		...
		MUJieripartChongzhiKingItem mujieripartChongzhiKingItem = this.listItem[position - 1];
		mujieripartChongzhiKingItem.AwardGiftGainState = JieriAwardGiftGainState.Gained;
	}
	...
	private List<MUJieripartChongzhiKingItem> listItem = new List<MUJieripartChongzhiKingItem>();
