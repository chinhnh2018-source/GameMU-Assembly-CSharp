...
public class LoversWishPartGetAward : UserControl
{
	...
	public void InitListItem(int id)
	{
		if (LoversWishPart.GetWishAwardDic()[id].GoodsTwo != null)
		{
			this.loadGoodsList(LoversWishPart.GetWishAwardDic()[id].GoodsOne + "|" + LoversWishPart.GetWishAwardDic()[id].GoodsTwo);
		}
		else
		{
			this.loadGoodsList(LoversWishPart.GetWishAwardDic()[id].GoodsOne);
		}
		this.GetMiaoShu(LoversWishPart.GetWishAwardDic()[id].Name);
	}
	...
	private void GetMiaoShu(string paiming)
	{
		...
		{
			"fac60d",
			string.Format(Global.GetLang("您的上周排名为{0}，奖励如下"), paiming)
		});
	}
