...
public class ZhuanxiangHuodongItem : UserControl
{
	protected override void InitializeComponent()
	{
		...
		{
			if (this.MUIType == UIType.Qianggou)
			{
				...
				{
					"e3b36c",
					string.Format(Global.GetLang("需要消耗{0}钻石，确定吗？"), this.m_SpecialActivityXML.Price.Split(new char[]
					{
						'|'
					})[0])
	...
	public int ChongZhiJiFen
	{
		...
		set
		{
			...
			this.mShengyuNum.text = string.Format(Global.GetLang("充值      {0}可获得{1}专享积分"), this.strArr[0], this.strArr[1]) + string.Format(Global.GetLang("          现有专享积分：{0}"), (this.m_ChongZhiJiFen < 0) ? 0 : this.m_ChongZhiJiFen);
		}
	}
	...
	public void InitItemData(UIType MUIType, ZhuanXiangType mZhuanXiangType, SpecActInfo mSpecActInfo, SpecialActivityXML mSpecialActivityXML, int mGroupID, string Price = "")
	{
		this.m_SpecialActivityXML = mSpecialActivityXML;
		this.mTitle.text = mSpecialActivityXML.Name;
		this.SetShengyuTimes(mGroupID, mSpecialActivityXML.Day);
		this.ActID = mSpecActInfo.ActID;
		this.m_Price = Price;
		if (null != this.m_BackImg.gameObject.GetComponent<UITexture>())
		{
			this.m_BackImg.URL = string.Format("NetImages/GameRes/Images/ZhuanXiang/zhuanxiang{0}.jpg", 1);
		}
		if (null != this.m_PeiTuImg.gameObject.GetComponent<UITexture>())
		{
			this.m_PeiTuImg.URL = string.Format("NetImages/GameRes/Images/ZhuanXiang/PeiTuType{0}.png", mSpecialActivityXML.Type);
		}
		switch (MUIType)
		{
		...
		case ZhuanXiangType.XianshiQianggou:
			...
			{
				this.mOldPriceLabel.text = Global.GetColorStringForNGUIText(new object[]
				{
					"fac60d",
					string.Format("{0}", mSpecialActivityXML.Price.Split(new char[]
					{
						'|'
					})[1])
				});
		...
		case ZhuanXiangType.ChongzhiDuihuan:
			...
			this.mShengyuNum.text = string.Format(Global.GetLang("充值      {0}可获得{1}专享积分"), this.strArr[0], this.strArr[1]) + string.Format(Global.GetLang("          现有专享积分：{0}"), (mSpecActInfo.ShowNum1 < 0) ? 0 : mSpecActInfo.ShowNum1);
			if (mSpecialActivityXML.Price.Split(new char[]
			{
				'|'
			}).Length == 1)
			{
				this.m_ChongZhiJiMax = int.Parse(mSpecialActivityXML.Price.Split(new char[]
				{
					'|'
				})[0]);
				...
				{
					"e3b36c",
					string.Format("{0}", Global.GetLang("专享积分"))
				});
				...
				if (int.Parse(mSpecialActivityXML.Price.Split(new char[]
				{
					'|'
				})[0]) > mSpecActInfo.ShowNum1)
				{
					this.m_XiaoHaoNum.text = Global.GetColorStringForNGUIText(new object[]
					{
						"ff0000",
						string.Format("{0}", Global.GetLang(mSpecialActivityXML.Price.Split(new char[]
						{
							'|'
						})[0]))
					});
		...
		case ZhuanXiangType.LeijiXiaofei:
		{
			...
			int num = mSpecialActivityXML.Goal.Split(new char[]
			{
				','
			})[0].SafeToInt32(0);
			...
			{
				"e3b36c",
				this.m_Str_Awardterm + string.Format(Global.GetLang("累计消费达到      {0}（{1} / {2}）"), num, (mSpecActInfo.ShowNum1 < num) ? mSpecActInfo.ShowNum1 : num, num)
			});
		...
		case ZhuanXiangType.ZhuanshengLevel:
			if (mSpecialActivityXML.Goal.Split(new char[]
			{
				','
			}).Length == 2)
			{
				...
				{
					"e3b36c",
					this.m_Str_Awardterm + string.Format(Global.GetLang("转生等级达到{0}转{1}级"), mSpecialActivityXML.Goal.Split(new char[]
					{
						','
					})[0], mSpecialActivityXML.Goal.Split(new char[]
					{
						','
					})[1])
				});
		...
		case ZhuanXiangType.ChibangLevel:
			...
			}).Length == 2)
			{
				...
				{
					"e3b36c",
					this.m_Str_Awardterm + string.Format(Global.GetLang("翅膀等级达到{0}阶{1}级"), mSpecialActivityXML.Goal.Split(new char[]
					{
						','
					})[0], mSpecialActivityXML.Goal.Split(new char[]
					{
		...
		{
			int num2 = mSpecialActivityXML.Goal.Split(new char[]
			{
				','
			})[0].SafeToInt32(0);
			this.mTiaojianLable.text = Global.GetColorStringForNGUIText(new object[]
		...
		{
			int num3 = mSpecialActivityXML.Goal.Split(new char[]
			{
				','
			})[0].SafeToInt32(0);
			this.mTiaojianLable.text = Global.GetColorStringForNGUIText(new object[]
			{
				"e3b36c",
				this.m_Str_Awardterm + string.Format(Global.GetLang("成就等级达到{0}（{1} / {2}）"), this.GetChengjiuName(num3), (mSpecActInfo.ShowNum1 < num3) ? mSpecActInfo.ShowNum1 : num3, num3)
			});
			NGUITools.SetActive(this.mShengyuNum.gameObject, false);
			break;
		}
		...
		{
			int num4 = mSpecialActivityXML.Goal.Split(new char[]
			{
				','
			})[0].SafeToInt32(0);
			this.mTiaojianLable.text = Global.GetColorStringForNGUIText(new object[]
			{
				"e3b36c",
				this.m_Str_Awardterm + string.Format(Global.GetLang("军衔等级达到{0}（{1} / {2}）"), this.GetJunxianName(num4), (mSpecActInfo.ShowNum1 < num4) ? mSpecActInfo.ShowNum1 : num4, num4)
			});
			NGUITools.SetActive(this.mShengyuNum.gameObject, false);
			break;
		}
		case ZhuanXiangType.MeilinLevel:
			...
			}).Length == 2)
			{
				...
				{
					"e3b36c",
					this.m_Str_Awardterm + string.Format(Global.GetLang("梅林之书等级达到{0}阶{1}级"), mSpecialActivityXML.Goal.Split(new char[]
					{
						','
					})[0], mSpecialActivityXML.Goal.Split(new char[]
					{
		...
		case ZhuanXiangType.ShengwuLevel:
			...
			{
				"e3b36c",
				this.m_Str_Awardterm + string.Format(Global.GetLang("圣物总等级达到{0}级"), mSpecialActivityXML.Goal.Split(new char[]
				{
					','
				})[0])
			});
		...
		case ZhuanXiangType.HunjieLevel:
			...
			}).Length == 2)
			{
				...
				{
					"e3b36c",
					this.m_Str_Awardterm + string.Format(Global.GetLang("婚戒等级达到{0}阶{1}级"), mSpecialActivityXML.Goal.Split(new char[]
					{
						','
					})[0], mSpecialActivityXML.Goal.Split(new char[]
					{
		...
		case ZhuanXiangType.ShouhushenLevel:
			...
			}).Length == 2)
			{
				...
				{
					"e3b36c",
					this.m_Str_Awardterm + string.Format(Global.GetLang("守护神等级达到{0}阶{1}级"), mSpecialActivityXML.Goal.Split(new char[]
					{
						','
					})[0], mSpecialActivityXML.Goal.Split(new char[]
					{
		...
		case ZhuanXiangType.ChaoJiZhiGou:
			...
			{
				this.m_ChongZhiID = int.Parse(mSpecialActivityXML.Price.Split(new char[]
				{
					'|'
				})[1]);
				this.m_ZhiGouID = int.Parse(mSpecialActivityXML.Price.Split(new char[]
				{
					'|'
				})[2]);
				...
				{
					"fac60d",
					Global.GetLang(string.Format(Global.GetLang("限时促销价：{0}"), Price))
				});
			}
			break;
		}
	...
	private void SetShengyuTimes(int GroupID, string day)
	{
		...
		if (Global.g_ZhuanXiangTime.TryGetValue(GroupID, ref specialActivityTime))
		{
			if (array.Length == 1)
			{
				if (string.IsNullOrEmpty(array[0]) || array[0].SafeToInt32(0) == -1)
				{
					DateTime.TryParse(specialActivityTime.ToDate, ref this.nextTime);
				}
				else
				{
					DateTime.TryParse(specialActivityTime.FromDate, ref this.nextTime);
					this.nextTime = this.nextTime.AddDays((double)array[0].SafeToInt32(0));
				}
			}
			else if (array.Length == 2)
			{
				DateTime.TryParse(specialActivityTime.FromDate, ref this.nextTime);
				this.nextTime = this.nextTime.AddDays((double)array[1].SafeToInt32(0));
			}
		}
	...
	protected void TickProc()
	{
		...
		if (num > correctLocalTime)
		{
			...
			this.mTime.text = Global.GetColorStringForNGUIText(new object[]
			{
				"ff0000",
				Global.GetLang("剩余时间：") + Global.GetTimeStrBySecEx((double)num2, false, -1)
			});
		}
		else if (num == correctLocalTime)
		{
			this.mTime.text = Global.GetLang("已经结束，等待刷新");
	...
	private void SetLingquState(SpecActInfo mSpecActInfo)
	{
	...
	public LingquState MLingquState
	{
		...
		set
		{
			...
			case (LingquState)2:
				this.btnConfim.isEnabled = false;
				this.btnConfim.normalSprite = "button_disable2";
				this.btnConfim.hoverSprite = "button_disable2";
				this.btnConfim.pressedSprite = "button_disable2";
	...
	public void SetShengyuNum(int mActID, int leftPurNum, int ShowNum1, int ShowNum2)
	{
		...
		if (Global.g_ZhuanXiangDic.TryGetValue(mActID, ref specialActivityXML))
		{
			if (specialActivityXML.Type == 1 && leftPurNum <= 0)
			{
				this.MLingquState = LingquState.Gained;
			}
			else if (specialActivityXML.Type == 2 && leftPurNum <= 0)
			{
				this.MLingquState = LingquState.Gained;
			}
			else if (specialActivityXML.Type == 14 && leftPurNum <= 0)
			{
				this.MLingquState = LingquState.Gained;
			}
			else if (specialActivityXML.Type != 1 && specialActivityXML.Type != 14 && specialActivityXML.Type != 2)
			{
				this.MLingquState = LingquState.Gained;
			}
			ZhuanXiangType type = (ZhuanXiangType)specialActivityXML.Type;
			switch (type)
			{
			case ZhuanXiangType.XianshiQianggou:
				this.mShengyuNum.text = Global.GetColorStringForNGUIText(new object[]
			...
			case ZhuanXiangType.ChongzhiDuihuan:
				...
				this.mShengyuNum.text = string.Format(Global.GetLang("充值      {0}可获得{1}专享积分"), this.strArr[0], this.strArr[1]) + string.Format(Global.GetLang("          现有专享积分：{0}"), (ShowNum1 < 0) ? 0 : ShowNum1);
				break;
			case ZhuanXiangType.LeijiXiaofei:
				this.mTiaojianLable.text = Global.GetColorStringForNGUIText(new object[]
				{
	...
	public TextBlock mOldPriceLabel;
	...
	private SpecialActivityXML m_SpecialActivityXML = default(SpecialActivityXML);
