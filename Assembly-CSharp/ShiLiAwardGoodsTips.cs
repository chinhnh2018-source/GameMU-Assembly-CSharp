...
public class ShiLiAwardGoodsTips : UserControl
{
	...
	private IEnumerator AddGoodListIcon(List<GoodsData> goodsList)
	{
		...
		if (goodsCount == 1)
		{
			this.AddGoodIcon(goodsList[0], new Vector3(0f, -60f, 0f));
		}
		else
		{
			int beginX = -220;
			...
			if (goodsCount < 5)
			{
				beginX += (int)((float)(5 - goodsCount) / 2f * interval);
			}
			for (int i = 0; i < 10; i++)
			{
				if (i >= 5)
