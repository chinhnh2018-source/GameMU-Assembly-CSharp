	...
	public class SweepWanmota
	{
		...
		private void Sweeping(object source, ElapsedEventArgs e)
		{
			lock (this.sweepClient)
			{
				...
				{
					this.StopSweeping();
					List<SingleLayerRewardData> list = SweepWanMotaManager.SummarySweepRewardInfo(this.sweepClient);
					List<SingleLayerRewardData> wanMoTaLayerRewardList = this.sweepClient.ClientData.LayerRewardData.WanMoTaLayerRewardList;
					this.sweepClient.ClientData.LayerRewardData.WanMoTaLayerRewardList = list;
					if (-1 == WanMoTaDBCommandManager.UpdateSweepAwardDBCommand(this.sweepClient, 0))
					{
						LogManager.WriteLog(2, "扫荡奖励汇总后，写到数据库失败", null, true);
					...
					else
					{
						...
						SweepWanMotaManager.UpdataSweepInfo(this.sweepClient, list);
					}
				}
				else
				{
