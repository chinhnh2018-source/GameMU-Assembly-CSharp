...
namespace GameServer.Logic.WanMota
{
	public class WanMoTaTopLayerManager : SingletonTemplate<WanMoTaTopLayerManager>
	{
		...
		public void OnClientPass(GameClient client, int layer)
		{
			...
			{
				string msgText = StringUtil.substitute(GLang.GetLang(572, new object[0]), new object[]
				{
			...
			if (layer >= 30)
			{
				...
				if (flag)
				{
					string msgText = StringUtil.substitute(GLang.GetLang(573, new object[0]), new object[]
					{
						Global.FormatRoleName(client, client.ClientData.RoleName)
					});
					Global.BroadcastRoleActionMsg(client, RoleActionsMsgTypes.HintMsg, msgText, true, GameInfoTypeIndexes.Hot, ShowGameInfoTypes.OnlySysHint, 0, 0, 100, 100);
