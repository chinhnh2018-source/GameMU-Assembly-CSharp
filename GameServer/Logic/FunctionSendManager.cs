...
namespace GameServer.Logic
{
	public class FunctionSendManager : IManager, ICmdProcessorEx, ICmdProcessor
	{
		...
		private void FunctionOperation(int roleID, int type, bool isOpen)
		{
			...
			{
				LogManager.WriteLog(9, string.Format("[ljl_博彩]CloseFunction{0}", ex.ToString()), null, true);
			}
		}
		...
		public void AddFunction(FunctionType type, int roleID)
		{
			try
			{
				...
				{
					List<int> list;
					if (this.FunctionDict.TryGetValue(type, out list))
					{
						if (list.Find((int x) => x == roleID) < 1)
						{
							list.Add(roleID);
						}
					}
					else
					{
						...
						this.FunctionDict.Add(type, list);
					}
				}
			}
			catch (Exception ex)
			{
				LogManager.WriteLog(9, string.Format("[ljl_关闭功能]type={1},ex={0}", ex.ToString(), type), null, true);
			}
		}
		...
		private void Send<T>(int roleID, FunctionType type, int nID, T data)
		{
			try
			{
				...
				{
					gameClient.sendCmd<T>(nID, data, false);
				}
			}
			catch (Exception ex)
			{
