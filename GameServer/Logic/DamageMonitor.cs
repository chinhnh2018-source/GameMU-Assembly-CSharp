		...
		public void Set(int mapCode, int RoleID)
		{
			if (!this.Get(mapCode, RoleID))
			{
				...
				else
				{
					...
					list.Add(RoleID);
					this.DictMonitorList[mapCode] = list;
				}
			}
		}
		...
		public void Out(GameClient client)
		{
			if (null != client)
			{
				if (this.Get(1, client.ClientData.RoleID))
				{
					...
					string text = string.Format("个人伤害统计,ticks={6}, mapcode={0}, roleid={1}, magiccode={2}, damage={3}, posx={4}, posy={5},vipexp={7}", new object[]
					{
					...
					{
						text += string.Format("LastEnemyID={0} LastEnemyName={1} LastEnemyPosX={2} LastEnemyPosY={3} dist={4:00} ", new object[]
						{
					...
					for (int i = 0; i < 12; i++)
					{
						...
						{
							text += string.Format("damagetype={0}", (DamageType)i);
						}
					}
