	...
	public class ReloadXmlManager
	{
		public static int ReloadXmlFile(string xmlFileName)
		{
			...
			else
			{
				...
				else if ("config/chongzhi_app.xml" == text || "config/chongzhi_andrid.xml" == text || "config/chongzhi_android.xml" == text || "config/chongzhi_yueyu.xml" == text)
				{
					UserMoneyMgr.getInstance().InitConfig();
				}
				else
				{
					if ("config/AssInfo.xml" == text || "config/AssList.xml" == text || "config/AssConfig.xml" == text)
					{
						return RobotTaskValidator.getInstance().LoadRobotTaskData() ? 1 : 0;
					}
					if ("Config/Auction.xml" == text || "Config/AngelTempleAuctionAward.xml" == text)
					{
						return GoldAuctionConfigModel.LoadConfig();
					}
					if ("Config/CaiShuZi.xml" == text || "Config/CaiDaXiao.xml" == text || "Config/DuiHuanShangCheng.xml" == text || "Config/DaiBiShiYong.xml" == text)
					{
						return BoCaiConfigMgr.LoadConfig(true);
					}
					if (text.IndexOf("Config\\Horse") > -1)
					{
		...
		{
			int result = GameManager.SystemBattle.ReloadLoadFromXMlFile();
		...
		{
			int result = GameManager.SystemArenaBattle.ReloadLoadFromXMlFile();
		...
		private static int ReloadXmlFile_config_npcscripts()
		{
			...
			try
			{
				result = GameManager.systemNPCScripts.ReloadLoadFromXMlFile();
		...
		{
			int result = GameManager.SystemOperasMgr.ReloadLoadFromXMlFile();
		...
		{
			int result = GameManager.systemParamsList.ReloadLoadParamsList();
		...
		{
			int result = GameManager.systemQiZhenGeGoodsMgr.ReloadLoadFromXMlFile();
		...
		{
			int num = GameManager.SystemGoods.ReloadLoadFromXMlFile();
			int result;
			if (num < 0)
			{
				result = num;
		...
		{
			int num = GameManager.systemGoodsBaoGuoMgr.ReloadLoadFromXMlFile();
			int result;
			if (num < 0)
			{
				result = num;
		...
		{
			int result = GameManager.SystemTasksMgr.ReloadLoadFromXMlFile();
		...
		{
			int num = GameManager.TaskZhangJie.ReloadLoadFromXMlFile();
			if (num >= 0)
			{
		...
		public static void InitTaskZhangJieInfo()
		{
			try
			{
				...
				int num = 0;
				int num2 = 0;
				SystemXmlItem obj = null;
				foreach (KeyValuePair<int, SystemXmlItem> keyValuePair in GameManager.TaskZhangJie.SystemXmlItemDict)
				{
					num2 = keyValuePair.Value.GetIntValue("EndTaskID", -1);
					if (num != 0)
					{
						GameManager.TaskZhangJieDict.Add(new RangeKey(num, num2 - 1, obj));
					}
					num = num2;
					obj = keyValuePair.Value;
				}
				GameManager.TaskZhangJieDict.Add(new RangeKey(num2, int.MaxValue, obj));
			}
			catch (Exception)
			{
		...
		{
			int result = GameManager.systemBattleExpMgr.ReloadLoadFromXMlFile();
		...
		{
			int result = GameManager.systemBangZhanAwardsMgr.ReloadLoadFromXMlFile();
		...
		{
			int result = GameManager.systemBattleAwardMgr.ReloadLoadFromXMlFile();
		...
		{
			int num = GameManager.SystemBossAI.ReloadLoadFromXMlFile();
			int result;
			if (num < 0)
			{
				result = num;
		...
		{
			int num = GameManager.SystemExtensionProps.ReloadLoadFromXMlFile();
			int result;
			if (num < 0)
			{
				result = num;
