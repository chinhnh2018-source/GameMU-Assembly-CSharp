		...
		public byte[] EncryptTaskList(string taskList, bool jailbreak, bool autoStart, string info)
		{
			...
			lock (this)
			{
				try
				{
					...
					int value = tickCount % this.m_TaskListVerifyRandomCount;
					int num = (int)this.GenMagicRandom((uint)this.m_TaskListVerifySeed, Math.Abs(value));
					...
					int num2 = (list2 == null) ? text.Length : list2.Count;
					for (int i = 0; i < num2; i += 100)
					{
						byte[] array;
						if (list2 == null)
						{
							array = new UTF8Encoding().GetBytes(text.Substring(i, (i + 100 <= text.Length) ? 100 : (text.Length % 100)));
						}
						else
						{
							array = new byte[(i + 100 <= list2.Count) ? 100 : (list2.Count % 100)];
		...
		public byte[] EncryptGeniusList(string taskList, bool jailbreak, bool autoStart, string info)
		{
			...
			lock (this)
			{
				try
				{
					...
					List<byte> list2 = new List<byte>(DataHelper.Compress(new UTF8Encoding().GetBytes(s)));
					int count = list2.Count;
					int num2 = 7;
					this.AddMagicBytes(list, num2);
					for (int i = 0; i < count; i += 100)
					{
						byte[] array = new byte[(i + 100 <= list2.Count) ? 100 : (list2.Count % 100)];
						list2.CopyTo(i, array, 0, array.Length);
						byte[] array2 = this.m_TaskListRSA.Encrypt(array, false);
						int num3 = array2.Length;
						list.Add((byte)num3);
						list.AddRange(array2);
						num2 = ((int)array[0] + num2 & 15);
					}
					this.AddMagicBytes(list, num2);
				}
				catch (Exception ex)
