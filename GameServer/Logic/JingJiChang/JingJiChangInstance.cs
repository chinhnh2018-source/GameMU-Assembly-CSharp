...
namespace GameServer.Logic.JingJiChang
{
	public class JingJiChangInstance : ScheduleTask
	{
		...
		public void run()
		{
			...
			if (num > this.startCDTime && num < this.startedTime && this.state == JingJiFuBenState.WAITING_CHANGEMAP_FINISH)
			{
				this.switchState(JingJiFuBenState.START_CD);
			}
			else if (num > this.startedTime && num < this.stopCDTime && this.state == JingJiFuBenState.START_CD)
			{
				this.switchState(JingJiFuBenState.STARTED);
			}
			else if (num > this.stopCDTime && num < this.stopedTime && this.state == JingJiFuBenState.STARTED)
			{
				this.switchState(JingJiFuBenState.STOP_TIMEOUT_CD);
			}
			else if (num > this.stopedTime && num < this.destroyTime && this.state == JingJiFuBenState.STOP_CD)
			{
				this.switchState(JingJiFuBenState.STOPED);
			}
			else if (num > this.destroyTime && this.state == JingJiFuBenState.STOPED)
			{
