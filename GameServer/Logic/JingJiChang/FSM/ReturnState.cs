...
namespace GameServer.Logic.JingJiChang.FSM
{
	internal class ReturnState : IFSMState
	{
		...
		private void changeAction(GActions action)
		{
			...
			{
				Point enemyTarget = this.owner.EnemyTarget;
				double direction = this.owner.Direction;
				List<object> all9Clients = Global.GetAll9Clients(this.owner);
				GameManager.ClientMgr.NotifyOthersDoAction(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, this.owner, this.owner.MonsterZoneNode.MapCode, this.owner.CopyMapID, this.owner.RoleID, (int)direction, (int)action, (int)this.owner.SafeCoordinate.X, (int)this.owner.SafeCoordinate.Y, (int)enemyTarget.X, (int)enemyTarget.Y, 114, all9Clients);
