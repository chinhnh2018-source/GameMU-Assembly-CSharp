...
namespace GameServer.Logic
{
	public class SafeClientData
	{
		...
		public int CurrentLifeV
		{
			...
			set
			{
				int num = value - this._CurrentLifeV;
				if ((long)value > ClientCmdCheck.MinLogAddLifeV && (long)num > ClientCmdCheck.MinLogAddLifeV)
				{
					string lifeLogString = ClientCmdCheck.GetLifeLogString(this.MapCode, this._CurrentLifeV, this.LifeV, num);
					if (!string.IsNullOrEmpty(lifeLogString))
					{
						lock (this)
						{
							this.AddLifeAlertList.Enqueue(lifeLogString);
						}
					}
				}
		...
		public Monster GetSummonMonster(int i)
		{
			...
			try
			{
				result = this.SummonMonstersList[i];
			}
			catch
			{
			}
		...
		public void AddAwardRecord(RoleAwardMsg type, List<GoodsData> goodsDataList, bool useOld = false)
		{
			if (goodsDataList != null)
			{
				...
				{
					this.AddAwardRecord(type, goodsData, useOld);
				}
			}
		}
		...
		public void AddAwardRecord(RoleAwardMsg type, string goodsDataListString, bool useOld = false)
		{
			if (!string.IsNullOrEmpty(goodsDataListString))
			{
				...
				awardsItemList.Add(goodsDataListString);
				this.AddAwardRecord(type, Global.ConvertToGoodsDataList(awardsItemList.Items, -1), useOld);
			}
		}
		...
		public void AddAwardRecord(RoleAwardMsg type, GoodsData goodsData, bool useOld = false)
		{
			if (null != goodsData)
			{
				...
				else
				{
					...
					list.Add(goodsData);
					this._TmpAwardRecordDict[type] = list;
				}
			}
		}
