	...
	public class CopyData
	{
		...
		public static bool Copy2AuctionItemS2C(GoldAuctionItem sData, out AuctionItemS2C rData, string Seach, int Color)
		{
			...
			try
			{
				...
				{
					List<int> list = Global.StringToIntList(Seach, '#');
					if (list.Find((int x) => x == goodsID) < 1)
					{
						return false;
					}
				}
				int equipColor = Global.GetEquipColor(rData.Goods);
				if (Color > 0 && (1 << equipColor - 1 & Color) == 0)
				{
					return false;
				}
				...
				AuctionConfig auctionConfig = GoldAuctionConfigModel.GetAuctionConfig(sData.AuctionSource);
				if (null != auctionConfig)
				{
					rData.MaxPrice = (long)auctionConfig.MaxPrice;
					rData.UnitPrice = (long)auctionConfig.UnitPrice;
				}
