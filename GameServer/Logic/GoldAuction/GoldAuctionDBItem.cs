...
namespace GameServer.Logic.GoldAuction
{
	...
	public class GoldAuctionDBItem
	{
		...
		public int GetMaxmDamageID()
		{
			try
			{
				AuctionRoleData auctionRoleData = new AuctionRoleData();
				foreach (AuctionRoleData auctionRoleData2 in this.RoleList)
				{
					if (auctionRoleData2.Value > auctionRoleData.Value)
					{
						auctionRoleData = auctionRoleData2;
					}
				}
				if (null != auctionRoleData)
				{
					return auctionRoleData.m_RoleID;
				}
			}
			catch
			{
