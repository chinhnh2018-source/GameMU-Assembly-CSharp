		...
		public static string CountMapIDOnlineNum()
		{
			...
			else
			{
				...
				for (int i = 0; i < GetMapcodeOnlineNumManager.arrCountMapcode.Length; i++)
				{
					...
					text += string.Format("{0},{1}", GetMapcodeOnlineNumManager.arrCountMapcode[i], GameManager.ClientMgr.GetMapClientsCount(GetMapcodeOnlineNumManager.arrCountMapcode[i]));
				}
				result = text;
			}
			return result;
