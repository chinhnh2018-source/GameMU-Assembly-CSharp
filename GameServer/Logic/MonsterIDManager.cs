...
namespace GameServer.Logic
{
	public class MonsterIDManager
	{
		...
		{
			long result = 2130771968L;
			lock (this.IdleIDList)
			{
				if (this.IdleIDList.Count > 0)
				{
					result = this.IdleIDList.ElementAt(0);
					this.IdleIDList.RemoveAt(0);
				}
				else
				{
					result = (this._MaxID += 1L);
				}
			}
			return result;
		}
		...
		public void PushBack(long id)
		{
			...
			{
				LogManager.WriteLog(2, string.Format("MonsterIDManager中的空闲ID数达到了{0}", this.IdleIDList.Count), null, true);
			}
		}
