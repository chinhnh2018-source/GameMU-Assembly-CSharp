...
namespace GameServer.Logic
{
	public class LuaManager
	{
		...
		{
			GameManager.MonsterZoneMgr.AddDynamicMonsters(client.ClientData.MapCode, monsterID, client.ClientData.CopyMapID, addNum, gridX, gridY, radius, 0, 0, null, null);
			return true;
		}
		...
		public string GetMapRecordXY(GameClient client, int recordIndex)
		{
			...
			int num3 = 0;
			string result;
			if (Global.GetMapRecordDataByField(client, recordIndex, out num, out num2, out num3))
			{
		...
		public string GetMapRecordMapName(GameClient client, int recordIndex)
		{
			...
			int num2 = 0;
			string result;
			if (Global.GetMapRecordDataByField(client, recordIndex, out mapCode, out num, out num2))
			{
		...
		public void GotoMapRecordXY(GameClient client, int recordIndex)
		{
			...
			if (Global.GetMapRecordDataByField(client, recordIndex, out num, out num2, out num3))
			{
				Point point = Global.GridToPixel(num, (double)num2, (double)num3);
				this.GotoMap(client, num, (int)point.X, (int)point.Y, -1);
			}
		}
		...
		{
			NPC npcfromConfig = NPCGeneralManager.GetNPCFromConfig(client.ClientData.MapCode, npcID, toX, toY, 0);
			if (null != npcfromConfig)
			{
				GameManager.ClientMgr.NotifyMySelfNewNPC(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, client, npcfromConfig);
			}
		}
		...
		{
			NPC npcfromConfig = NPCGeneralManager.GetNPCFromConfig(mapCode, npcID, toX, toY, 0);
			if (null != npcfromConfig)
			{
		...
		{
			int num = Global.FindBossFuBenIDByRoleLevel(client.ClientData.Level);
			string result;
			if (num > 0)
			{
				FuBenData fuBenData = Global.GetFuBenData(client, num);
				if (null != fuBenData)
				{
					int num2;
					result = string.Format("{0}", Math.Max(0, Global.GetBossFuBenCanFreeEnterNum(client) - Global.GetFuBenEnterNum(fuBenData, out num2)) + Global.GetBossFuBenCanExtEnterNum(client));
				}
				else
				{
		...
		{
			int num = Global.EnterBossFuBen(client);
			if (-1 == num)
			{
			...
			{
				this.Error(client, string.Format(GLang.GetLang(425, new object[0]), num), 0);
			}
		}
		...
		public string GetErGuoTouBufferName(GameClient client)
		{
			...
			else if (Global.IsBufferDataOver(bufferDataByID, 0L))
			{
				result = GLang.GetLang(426, new object[0]);
			}
			else
			{
				long num = (long)((ulong)-1 & (ulong)(bufferDataByID.BufferVal >> 32));
				result = Global.GetGoodsNameByID((int)num);
			}
			return result;
		}
		...
		public string GetErGuoTouBufferExperience(GameClient client)
		{
			...
			else
			{
				...
				if (null != roleSitExpItem)
				{
					...
					num += Global.ProcessTeamZhuFuExperience(client);
					double num2 = (double)(bufferDataByID.BufferVal & (long)((ulong)-1)) - 1.0;
					num += num2;
					result = ((int)((double)experience * num)).ToString();
				}
				else
				{
					result = "0";
		...
		{
			List<GoodsData> buChangGoodsDataList = BuChangManager.GetBuChangGoodsDataList(client);
			string result;
			if (null == buChangGoodsDataList)
			{
				result = "";
			}
			else
			{
				...
				for (int i = 0; i < buChangGoodsDataList.Count; i++)
				{
					if (i > 0)
					{
						stringBuilder.Append(" ");
					}
					stringBuilder.AppendFormat("{0}({1})", Global.GetGoodsNameByID(buChangGoodsDataList[i].GoodsID), buChangGoodsDataList[i].GCount);
				}
