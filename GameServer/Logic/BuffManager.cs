	...
	public class BuffManager : IEventListener
	{
		public static void InitConfig()
		{
			lock (BuffManager.mutex_config)
			{
				...
				string paramValueByName = GameManager.systemParamsList.GetParamValueByName("ZSPLHXZAttackInjurePercent");
				if (!string.IsNullOrEmpty(paramValueByName))
				{
					...
					{
						BuffManager.ZSPLHXZAttackInjurePercent.Add(Global.SafeConvertToDouble(array[i]));
					}
				}
			}
		}
		...
		static BuffManager()
		{
			foreach (KeyValuePair<int, int> keyValuePair in BuffManager.BufferId2Flags)
			{
				BuffManager.BuffId2FlagsDict[keyValuePair.Key].isUpdateByMapCode = ((keyValuePair.Value & 4) != 0);
				BuffManager.BuffId2FlagsDict[keyValuePair.Key].isUpdateByTime = ((keyValuePair.Value & 1) != 0);
				BuffManager.BuffId2FlagsDict[keyValuePair.Key].isUpdateByVip = ((keyValuePair.Value & 2) != 0);
			}
		}
		...
		public BuffManager()
		{
			...
			foreach (KeyValuePair<int, int> keyValuePair in BuffManager.BufferId2Flags)
			{
				this.BuffListArray[keyValuePair.Key].flags = keyValuePair.Value;
			}
		}
		...
		public void UpdateImmediately(GameClient client, int id, long nowTicks)
		{
			lock (this.mutex)
			{
				bool flag2 = this.BuffListArray[id].endTicks > nowTicks;
				if (flag2 != this.BuffListArray[id].enabledByTime)
				{
					this.BuffListArray[id].enabledByTime = flag2;
					int flags = this.BuffListArray[id].flags;
					this.BuffListArray[id].enabled = flag2;
		...
		private void OnBaTiStateChange(GameClient client, bool active, BuffItemData buffItemData)
		{
			if (active)
			{
				if (buffItemData.buffVal >= 30L)
				{
					...
					GameManager.ClientMgr.NotifyRoleStatusCmd(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, client, 12, TimeUtil.NOW(), (int)(buffItemData.endTicks - buffItemData.startTicks), moveSpeed);
				}
			...
			else
			{
				if (buffItemData.buffId >= 30)
				{
					double moveSpeed = RoleAlgorithm.GetMoveSpeed(client);
					client.ClientData.MoveSpeed = moveSpeed;
					GameManager.ClientMgr.NotifyRoleStatusCmd(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, client, 12, TimeUtil.NOW(), 0, moveSpeed);
				}
				if (buffItemData.buffVal >= 50L)
				{
