	...
	public class RoleSpeedControl
	{
		...
		public double LastSlowRate
		{
			get
			{
				...
				int gameConfigItemInt = GameManager.GameConfigMgr.GetGameConfigItemInt("punish-speed-secs", 5);
				double result;
				if (num - this._LastSlowRateTicks < (long)(gameConfigItemInt * 1000))
				{
		...
		{
			RoleSpeedItem item = new RoleSpeedItem
			{
				MapCode = mapCode,
				X = x,
				Y = y,
			...
			this.RoleSpeedItemList.Add(item);
		}
