	...
	public class JunQiItem : IObject
	{
		...
		public int AttackedRoleID
		{
			...
			set
			{
				lock (this)
				{
					...
					{
						this._LastAttackedTick = num;
					}
					else if (num - this._LastAttackedTick >= 10000L)
					{
						this._LastAttackedTick = num;
		...
		public void AddAttacker(int roleID, int injured, Monster DSPet = null)
		{
			lock (this._AttackerLogDict)
			{
				...
				JunQiAttackerLog junQiAttackerLog = null;
				if (!this._AttackerLogDict.TryGetValue(roleID, out junQiAttackerLog))
				{
					junQiAttackerLog = new JunQiAttackerLog();
					junQiAttackerLog.RoleId = roleID;
					this._AttackerLogDict[roleID] = junQiAttackerLog;
				}
				junQiAttackerLog.LastAttackMs = this._LastLogAttackerTicks;
				if (null == DSPet)
				{
					junQiAttackerLog.TotalInjured += (long)injured;
				}
				else
				{
					junQiAttackerLog.TotalInjuredByPet += (long)injured;
				}
			}
		}
		...
		public int GetAttackerFromList()
		{
			...
			lock (this._AttackerLogDict)
			{
				foreach (JunQiAttackerLog junQiAttackerLog in this._AttackerLogDict.Values)
				{
					if (num2 - junQiAttackerLog.LastAttackMs < 30000L)
					{
						if (junQiAttackerLog.TotalInjured > num || junQiAttackerLog.TotalInjuredByPet > num)
						{
							num = ((junQiAttackerLog.TotalInjured > junQiAttackerLog.TotalInjuredByPet) ? junQiAttackerLog.TotalInjured : junQiAttackerLog.TotalInjuredByPet);
							result = junQiAttackerLog.RoleId;
						}
					}
				}
			}
