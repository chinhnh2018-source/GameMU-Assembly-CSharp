	...
	public class FakeRoleItem : IObject
	{
		...
		public void AddAttacker(int roleID, int injured)
		{
			lock (this._AttackerDict)
			{
				...
				int num = 0;
				this._AttackerDict.TryGetValue(roleID, out num);
				this._AttackerDict[roleID] = num + injured;
		...
		public int GetAttackerFromList()
		{
			...
			lock (this._AttackerDict)
			{
				foreach (int num3 in this._AttackerDict.Keys)
				{
					long num4 = 0L;
					this._AttackerTicksDict.TryGetValue(num3, out num4);
					if (num2 - num4 < 30000L)
					{
						int num5 = this._AttackerDict[num3];
						if (num5 > num)
						{
							num = num5;
							result = num3;
