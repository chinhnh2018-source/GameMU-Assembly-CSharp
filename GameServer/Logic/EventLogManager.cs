...
namespace GameServer.Logic
{
	public class EventLogManager
	{
			...
			try
			{
				if (GameManager.FlagEnableMoneyEventLog)
				{
					if (LogFilterConfig.LogMoneyTypeLog((int)moneyType))
					{
						...
						string msg2 = string.Format("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\t{9}\t{10}", new object[]
						{
							1,
							serverId,
							zoneId,
						...
						EventLogManager.SystemRoleEvents[73].AddEvent(msg2, EventLevels.Important);
					}
				}
			}
			catch (Exception ex)
				...
				if (addValue != 0)
				{
					if (GameManager.FlagEnableGoodsEventLog)
					{
						if (LogFilterConfig.LogGoodsIdLog(goodsId))
						{
							...
							string msg2 = string.Format("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\t{9}\t{10}\t{11}", new object[]
							{
								2,
								serverId,
								zoneID,
							...
							EventLogManager.SystemRoleEvents[74].AddEvent(msg2, EventLevels.Important);
						}
					}
				}
			}
		...
		public static void AddRoleInitGameEvent(GameClient client, string hid)
		{
			...
			string text2 = Global.GetSocketRemoteEndPoint(client.ClientSocket, false).Replace(":", ".");
		...
		public static void AddRoleLogoutEvent(GameClient client)
		{
			string text = GameManager.OnlineUserSession.FindUserID(client.ClientSocket);
			string text2 = Global.GetSocketRemoteEndPoint(client.ClientSocket, false).Replace(":", ".");
			string text3 = client.ClientData.LastLoginTime.ToString("yyyy-MM-dd HH:mm:ss");
			EventLogManager.SystemRoleEvents[1].AddImporEvent(new object[]
			{
		...
		public static void AddCreateRoleEvent(string userID, int zoneID, string strCmdResult, string ip, string hid)
		{
			try
			{
				string[] array = strCmdResult.Split(new char[]
				{
					':'
				});
				string b = array[0];
			...
			{
				LogManager.WriteLog(2, string.Format("AddCreateRoleEvent Exception {0} {1}", userID, strCmdResult), null, true);
			}
		}
		...
		{
			string text = "";
			foreach (int num in client.ClientData.OccupationList)
			{
		...
		public static void AddGameEvent(LogRecordType logRecordType, params object[] args)
		{
			try
			{
				if (GameManager.FlagEnableGameEventLog)
				{
					...
					string text = string.Format("{0}\t{1}", (int)logRecordType, serverId);
					if (logRecordType == LogRecordType.Json)
					{
						if (args[0].GetType() == typeof(Hashtable))
						{
							text = text + "\t" + MUJson.jsonEncode(args[0]);
						}
						else
						{
							...
							{
								hashtable.Add(args[i], args[1]);
							}
							text = text + "\t" + MUJson.jsonEncode(hashtable);
						}
					}
					else
					{
					...
					EventLogManager.SystemRoleEvents[75].AddEvent(text, EventLevels.Important);
				}
			}
			catch (Exception ex)
			{
		...
		public static void AddRoleEvent(GameClient client, OpTypes optType, OpTags optTag, LogRecordType logRecordType, params object[] args)
		{
			try
			{
				if (GameManager.FlagEnableOperatorEventLog)
				{
					...
					EventLogManager.SystemRoleEvents[76].AddEvent(text, EventLevels.Important);
				}
			}
			catch (Exception ex)
			{
		...
		public static void AddRoleSkillEvent(GameClient client, SkillLogTypes optType, LogRecordType logRecordType, params object[] args)
		{
			try
			{
				if (GameManager.FlagEnableRoleSkillLog)
				{
					...
					EventLogManager.SystemRoleEvents[77].AddEvent(text, EventLevels.Important);
				}
			}
			catch (Exception ex)
			{
		...
		public static void AddPetSkillEvent(GameClient client, LogRecordType logRecordType, EPetSkillLog logType, params object[] args)
		{
			try
			{
				if (GameManager.FlagEnablePetSkillLog)
				{
					...
					EventLogManager.SystemRoleEvents[78].AddEvent(text, EventLevels.Important);
				}
			}
			catch (Exception ex)
			{
		...
		public static void AddUnionPalaceEvent(GameClient client, LogRecordType logRecordType, params object[] args)
		{
			try
			{
				if (GameManager.FlagEnableUnionPalaceLog)
				{
					...
					EventLogManager.SystemRoleEvents[79].AddEvent(text, EventLevels.Important);
				}
			}
			catch (Exception ex)
			{
		...
		{
			string text = string.Format("{0}", (int)type);
		...
		{
			string text = EventLogManager.MakeGoodsDataPropString(goodsDataList);
