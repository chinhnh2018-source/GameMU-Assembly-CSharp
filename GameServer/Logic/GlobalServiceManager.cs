	...
	public class GlobalServiceManager
	{
		...
		public static void initialize()
		{
			...
			lock (GlobalServiceManager.Scene2ManagerDict)
			{
				...
				{
					foreach (IManager manager in list)
					{
						bool flag2 = true;
						try
						{
							flag2 = (flag2 && manager.initialize());
							IManager2 manager2 = manager as IManager2;
							if (null != manager2)
							{
								flag2 = (flag2 && manager2.initialize(GameCoreInterface.getinstance()));
							}
						}
						catch (Exception ex)
						{
						...
						if (GameManager.ServerStarting && !flag2)
						{
							LogManager.WriteLog(1000, string.Format("执行{0}.initialize()失败,按任意键继续启动!", manager.GetType()), null, true);
							Console.ReadKey();
						}
					}
				}
			...
			lock (GlobalServiceManager.Scene2ManagerDict)
			{
				foreach (List<IManager> list in GlobalServiceManager.Scene2ManagerDict.Values)
				{
					foreach (IManager manager in list)
					{
						try
						{
							bool flag2 = manager.startup();
							if (GameManager.ServerStarting && !flag2)
							{
								LogManager.WriteLog(1000, string.Format("初始化{0}.startup()失败,按任意键忽略此错误并继续启动服务器!", manager.GetType()), null, true);
								Console.ReadKey();
							}
						}
						catch (Exception ex)
		...
		public static bool AddCopyScenes(GameClient client, CopyMap copyMap, SceneUIClasses sceneType)
		{
			...
			{
				List<IManager> list;
				if (GlobalServiceManager.Scene2ManagerDict.TryGetValue(sceneType, out list))
				{
					foreach (IManager manager in list)
					{
						ICopySceneManager copySceneManager = manager as ICopySceneManager;
						if (null != copySceneManager)
						{
							try
							{
								flag = (flag && copySceneManager.AddCopyScenes(client, copyMap, sceneType));
							}
							catch (Exception ex)
							{
								LogManager.WriteException(ex.ToString());
				...
				if (GlobalServiceManager.Scene2ManagerDict.TryGetValue(sceneType, out list))
				{
					foreach (IManager manager in list)
					{
						...
						if (null != copySceneManager)
						{
							try
							{
								flag = (flag && copySceneManager.RemoveCopyScene(copyMap, sceneType));
							}
							catch (Exception ex)
							{
								LogManager.WriteException(ex.ToString());
