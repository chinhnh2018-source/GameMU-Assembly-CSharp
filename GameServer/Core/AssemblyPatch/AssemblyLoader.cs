	...
	internal class AssemblyLoader : MarshalByRefObject
	{
		...
		public bool LoadAssembly(string assemblyName)
		{
			try
			{
				...
				{
					IEnumerable<string> enumerable = Directory.EnumerateFiles(Directory.GetCurrentDirectory(), assemblyName + ".dll");
		...
		public MethodLoader FindMethod(string fullClassName, string methodName)
		{
			...
			string key = string.Format("{0}.{1}", fullClassName, methodName);
			MethodLoader result;
			if (this.dictMethodLoader.TryGetValue(key, out methodLoader))
			{
				result = methodLoader;
			}
			else
		...
		public void AddMethod(string fullClassName, string methodName, MethodLoader methodLoader)
		{
			...
			{
				string key = string.Format("{0}.{1}", fullClassName, methodName);
				this.dictMethodLoader.Add(key, methodLoader);
			}
			catch
			{
				LogManager.WriteLog(2, string.Format("AssemblyLoader::AddMethod Error, fullClassName={0} methodName={1}", fullClassName, methodName), null, true);
			}
		}
		...
		public bool LoadMethod(string fullClassName, string methodName)
		{
			...
			{
				LogManager.WriteLog(2, string.Format("AssemblyLoader::LoadMethod Error, fullClassName={0}, methodName={1}", fullClassName, methodName), ex, false);
				return false;
			}
			return true;
		}
