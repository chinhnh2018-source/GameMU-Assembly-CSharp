	...
	public class ActivityIconStateData : IProtoBuffData
	{
		public int fromBytes(byte[] data, int offset, int count)
		{
			...
			if (num > 0)
			{
				...
				while (i < count)
				{
					...
					this.arrIconState[num++] = (ushort)ProtoUtil.IntMemberFromBytes(data, wt, ref result, ref i);
				}
			}
			return result;
		}
		...
		public byte[] toBytes()
		{
			...
			if (this.arrIconState != null && this.arrIconState.Length > 0)
			{
				for (int i = 0; i < this.arrIconState.Length; i++)
				{
					ProtoUtil.IntMemberToBytes(array, 1, ref num2, (int)this.arrIconState[i], true, 0);
				}
			}
			return array;
		}
