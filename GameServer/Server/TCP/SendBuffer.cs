...
namespace Server.TCP
{
	public class SendBuffer
	{
		...
		public void Reset2()
		{
			...
			{
				int num = this._CurrentBufferSize - this._UsedBufferSize;
				if (num > 0)
				{
					Array.Copy(this._Buffer, this._UsedBufferSize, this._Buffer, 0, num);
				}
				this._CurrentBufferSize = num;
		...
		private bool TrySend(TMSKSocket s, bool force = false)
		{
			...
			bool result;
			if (!this._IsSendding && this._CurrentBufferSize > 0 && (force || num - this.AddFirstPacketTicks >= SendBuffer._SendDataIntervalTicks))
			{
				this._IsSendding = true;
				Interlocked.Exchange(ref this._SendTimeoutTickCount, num + SendBuffer._SendDataTimeOutTicks);
		...
		private bool TrySend2(TMSKSocket s, long ticks, bool force = false)
		{
			...
			lock (this.BufferLock)
			{
				...
				{
					this._IsSendding = true;
					num = (this._UsedBufferSize = this._CurrentBufferSize);
			...
			if (num > 0)
			{
				Global._TCPManager.MySocketListener.SendData(s, this._Buffer, 0, num, this._MemoryBlock, this);
				result = true;
			}
			else
			{
		...
		{
			long num = TimeUtil.NOW();
			if (num - this.AddFirstPacketTicks >= SendBuffer._SendDataIntervalTicks || num < this.AddFirstPacketTicks)
			{
				if (Monitor.TryEnter(this.SendLock))
				{
					try
					{
						return this.TrySend2(s, num, force);
					}
					finally
					{
		...
		protected bool IsSendTimeOut()
		{
			...
			{
				long num = TimeUtil.NOW();
				result = (num - this._LastSendDataTicks >= SendBuffer._SendDataTimeOutTicks && this._LastSendDataTicks > 0L);
			}
			return result;
		}
		...
		public bool CanSend(int bytesCount, int cmdID, out int canNotSendReason, byte[] buffer, TMSKSocket s)
		{
			...
			lock (this.BufferLock)
			{
				if (num > Interlocked.Read(ref this._SendTimeoutTickCount) && this.CanDiscardCmd(cmdID))
				{
					canNotSendReason = 0;
					result = false;
				}
		...
		{
			int packetCmdID = (int)tcpOutPacket.PacketCmdID;
			long num = TimeUtil.NOW();
			byte[] packetBytes = tcpOutPacket.GetPacketBytes();
			int packetDataSize = tcpOutPacket.PacketDataSize;
			bool result;
			if (packetBytes == null || packetDataSize > packetBytes.Length)
			{
				result = false;
			}
			else
			{
				TCPManager.RecordCmdOutputDataSize(packetCmdID, (long)packetDataSize);
				int num2 = this._MaxBufferSize - packetDataSize;
				bool flag = 110 == packetCmdID;
				lock (this.BufferLock)
				{
					if (this._CurrentBufferSize >= num2)
					{
						canNotSendReason = 1;
						return false;
					}
					if (0 == this._CurrentBufferSize)
					{
						this._AddFirstPacketTicks = num;
					}
					if (num > Interlocked.Read(ref this._SendTimeoutTickCount) && this.CanDiscardCmd(packetCmdID))
					{
						canNotSendReason = 0;
						return false;
					}
					...
					DataHelper.CopyBytes(this._Buffer, this._CurrentBufferSize, packetBytes, 0, packetDataSize);
					this._CurrentBufferSize += packetDataSize;
				}
				if (Monitor.TryEnter(this.SendLock))
				{
