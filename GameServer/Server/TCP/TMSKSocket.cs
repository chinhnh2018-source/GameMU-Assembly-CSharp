	...
	public class TMSKSocket : IDisposable
	{
		...
		public void SetAcceptIp()
		{
			if (null != this.m_Socket)
			{
				try
				{
					byte[] addressBytes = ((IPEndPoint)this.m_Socket.RemoteEndPoint).Address.GetAddressBytes();
					this._AcceptIpAsInt = (long)(((ulong)addressBytes[0] << 24) + ((ulong)addressBytes[1] << 16) + ((ulong)addressBytes[2] << 8) + (ulong)addressBytes[3]);
				}
				catch
				{
				}
