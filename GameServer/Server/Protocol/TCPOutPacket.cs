...
namespace Server.Protocol
{
	public class TCPOutPacket : IDisposable
	{
		...
		public bool FinalWriteData(byte[] buffer, int offset, int count)
		{
			...
			{
				LogManager.WriteLog(2, string.Format("TCP发出命令包不能被重复写入数据, 命令ID: {0}", this.PacketCmdID), null, true);
				result = false;
			}
			else
			{
				if (6 + count >= 131072)
				{
					if (count >= 1048576)
					{
						LogManager.WriteLog(2, string.Format("TCP命令包长度:{0}, 最大不能超过: {1}, 命令ID: {2}", count, 1048576, this.PacketCmdID), null, true);
						return false;
					}
					LogManager.WriteLog(2, string.Format("TCP命令包长度:{0}, 警告长度: {1}, 命令ID: {2}", count, 131072, this.PacketCmdID), null, true);
				}
		...
		private void Final()
		{
			int value = this._PacketDataSize + 2;
			DataHelper.CopyBytes(this.PacketBytes, 0, BitConverter.GetBytes(value), 0, 4);
