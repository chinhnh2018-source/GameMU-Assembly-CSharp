	...
	public class CheckHelper
	{
		public static XElement LoadXml(string filePath, bool isFatal = true)
		{
			...
			if (!File.Exists(filePath))
			{
				if (isFatal)
				{
					LogManager.WriteLog(1000, string.Format("加载[{0}]时出错!!!文件不存在", filePath), null, true);
				}
				else
				{
					LogManager.WriteLog(2, string.Format("加载[{0}]时出错!!!文件不存在", filePath), null, true);
				}
				result = null;
			}
			else
		...
		public static bool CheckTCPCmdFields(TMSKSocket socket, int nID, byte[] data, int count, string[] fields, int length)
		{
			...
			{
				LogManager.WriteLog(2, string.Format("指令参数个数错误, CMD={0}, Client={1}, Recv={2}", (TCPGameServerCmds)nID, Global.GetSocketRemoteEndPoint(socket, false), fields.Length), null, true);
				result = false;
			}
			else
			{
		...
		public static bool CheckCmdUser(GameClient client, int nID, string[] cmdParams)
		{
			...
			bool result;
			if (client == null || client.strUserID != text)
			{
				LogManager.WriteLog(2, string.Format("根据userID定位GameClient对象失败, CMD={0}, Client={1}, userID={2}", (TCPGameServerCmds)nID, Global.GetSocketRemoteEndPoint(client.ClientSocket, false), text), null, true);
				result = false;
			}
			else
			{
