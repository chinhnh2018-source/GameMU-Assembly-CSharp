...
public static class ModuleRegistry
{
    public static readonly List<EventModule> Modules = new()
    {
        ...
        // ── Mới: Quà lễ hội JieRi (44 files) ───────────────────────────
        new() { Id="jieri", Name="Quà lễ hội (JieRi)", Icon="balloon-heart",
            Description="Tất cả quà lễ hội: đăng nhập, tích lũy nạp, vua nạp, boss công thành...",
            Categories=new[]{ "Quà lễ hội" } },

        // ── Mới: Hợp phục / Gộp server (12 files) ─────────────────────
        new() { Id="hefu", Name="Hợp phục / Gộp server", Icon="people",
            Description="Sự kiện khi gộp server: đăng nhập, nạp hoàn, shop giới hạn, boss...",
            Categories=new[]{ "Hợp phục / Gộp server" } },

        // ── Mới: Quà & Mã quà (GiftCode + Daily Award) ────────────────
        new() { Id="giftcode", Name="Gift Code & Quà", Icon="ticket-perforated",
            Description="Mã quà tặng (GiftCodeNew.xml) và đổi điểm hoạt động (DailyActiveAward)",
    ...
    /// Tìm module lớn chứa một category cụ thể (dùng để highlight sidebar khi đang ở trang File/Edit).
