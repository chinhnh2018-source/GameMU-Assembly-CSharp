...
namespace KF.Remoting
{
	...
	public class KuaFuCopyService : MarshalByRefObject, IKuaFuCopyService
	{
		...
		private void ThreadProc(object state)
		{
			...
			for (;;)
			{
				try
				{
					...
					if (num >= this.LastSaveServerStateMs + 30000L)
					{
						this.LastSaveServerStateMs = num;
						this.dbMgr.SaveCopyTeamAnalysisData(this.teamMgr.BuildAnalysisData());
					}
					this.teamMgr.Update();
					AsyncDataItem[] array = this.teamMgr.PopAsyncEvent();
					ClientAgentManager.Instance().BroadCastAsyncEvent(this.GameType, array);
					this.dbMgr.CheckLogAsyncEvents(array);
					SpecPriorityActivityMgr.Instance().Update(dateTime);
					int num2 = (int)(TimeUtil.NowDateTime() - dateTime).TotalMilliseconds;
					this.dbMgr.SaveCostTime(num2);
					num2 = 1600 - num2;
					if (num2 < 50)
					{
						num2 = 50;
		...
		public int InitializeClient(KuaFuClientContext clientInfo)
		{
			...
			try
			{
				if (clientInfo.GameType == this.GameType && clientInfo.ServerId != 0)
				{
		...
		public readonly GameTypes GameType = 8;
