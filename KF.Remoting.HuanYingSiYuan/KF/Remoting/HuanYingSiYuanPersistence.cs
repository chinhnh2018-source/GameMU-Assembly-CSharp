	...
	public class HuanYingSiYuanPersistence
	{
		...
		public void InitConfig()
		{
			try
			{
				...
				Consts.HuanYingSiYuanRoleCountTotal = (int)ConfigHelper.GetElementAttributeValueLong(xelement, "add", "key", "HuanYingSiYuanRoleCountTotal", "value", 16L);
				Consts.HuanYingSiYuanRoleCountPerSide = (int)ConfigHelper.GetElementAttributeValueLong(xelement, "add", "key", "HuanYingSiYuanRoleCountPerSide", "value", 8L);
				this.SignUpWaitSecs1 = (int)ConfigHelper.GetElementAttributeValueLong(xelement, "add", "key", "SignUpWaitSecs1", "value", 30L);
				this.SignUpWaitSecs2 = (int)ConfigHelper.GetElementAttributeValueLong(xelement, "add", "key", "SignUpWaitSecs2", "value", 60L);
		...
		public void SaveCostTime(int ms)
		{
			try
			{
				...
				{
					LogManager.WriteLog(1, "HuanYingSiYuan 执行时间(ms):" + ms, null, true);
				}
			}
			catch
			{
		...
		{
			string sqlCmd = string.Format("INSERT INTO t_hysy_0(`id`,`serverid`,`fubensid`,`createtime`,`rolenum`) VALUES({0},{1},{2},'{3}',{4});", new object[]
			{
				gameId,
				kfSrvId,
				fubenSeqId,
