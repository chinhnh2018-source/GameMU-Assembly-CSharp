...
namespace KF.Remoting
{
	internal class CoupleArenaJoinDataUtil
	{
		public CoupleArenaJoinData Create()
		{
			...
			coupleArenaJoinData.JoinSeq = this.CurrJoinSeq;
			this.CurrJoinSeq += 1U;
			return coupleArenaJoinData;
		}
		...
		{
			uint key;
			if (this.RoleJoinSeqDict.TryGetValue(roleId, out key))
			{
				CoupleArenaJoinData result;
				if (this.JoinDataDict.TryGetValue(key, out result))
				{
					return result;
				}
			}
