...
namespace KF.Remoting
{
	internal class ZhengBaStateMachine
	{
		...
		{
			ZhengBaStateMachine.StateHandler stateHandler = this.Handlers[(int)this._CurrState];
			if (stateHandler != null)
			{
				stateHandler.Leave(now);
			}
			this._CurrState = state;
			ZhengBaStateMachine.StateHandler stateHandler2 = this.Handlers[(int)this._CurrState];
			this._CurrStateEnterTicks = now.Ticks;
			if (stateHandler2 != null)
			{
				stateHandler2.Enter(now);
			}
		}
