...
namespace KF.Remoting
{
	public class KuaFuLueDuoStateMachine
	{
		...
		{
			KuaFuLueDuoStateMachine.StateHandler stateHandler = this.Handlers[(int)this._CurrState];
			if (stateHandler != null)
			{
				stateHandler.Leave(now, param);
			}
			this._CurrState = state;
			KuaFuLueDuoStateMachine.StateHandler stateHandler2 = this.Handlers[(int)this._CurrState];
			this._CurrStateEnterTicks = now.Ticks;
			if (stateHandler2 != null)
			{
				stateHandler2.Enter(now, param);
			}
		}
