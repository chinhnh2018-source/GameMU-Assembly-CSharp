...
namespace KF.Remoting
{
	internal class BHMatchStateMachine
	{
		...
		{
			BHMatchStateMachine.StateHandler stateHandler = this.Handlers[(int)this._CurrState];
			if (stateHandler != null)
			{
				stateHandler.Leave(now, param);
			}
			this._CurrState = state;
			BHMatchStateMachine.StateHandler stateHandler2 = this.Handlers[(int)this._CurrState];
