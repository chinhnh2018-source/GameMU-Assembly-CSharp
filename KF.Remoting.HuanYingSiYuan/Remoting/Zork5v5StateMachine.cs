...
namespace Remoting
{
	internal class Zork5v5StateMachine
	{
		...
		{
			Zork5v5StateMachine.StateHandler stateHandler = this.Handlers[(int)this._CurrState];
			if (stateHandler != null)
			{
				stateHandler.Leave(now, param);
			}
			this._CurrState = state;
			Zork5v5StateMachine.StateHandler stateHandler2 = this.Handlers[(int)this._CurrState];
