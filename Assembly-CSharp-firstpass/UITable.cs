...
public class UITable : MonoBehaviour
{
	...
	public static int SoryByPosY(Transform a, Transform b)
	{
		float y = a.localPosition.y;
		float y2 = b.localPosition.y;
		return y.CompareTo(y2);
	}

	public void Clear()
	{
		foreach (Transform transform in this.mChildren)
		{
	...
	public List<Transform> children
	{
		get
		{
			if (this.mChildren.Count == 0)
			{
				...
				else if (this.sortedByPosY)
				{
					this.mChildren.Sort(new Comparison<Transform>(UITable.SoryByPosY));
				}
			}
	...
	private void RepositionVariableSize(List<Transform> children)
	{
		...
		while (j < count2)
		{
			...
			localPosition.x += bounds2.min.x - bounds3.min.x;
			if (this.IsPosYFixed)
			{
			...
			else if (this.direction == UITable.Direction.Down)
			{
				localPosition.y = -num2 - bounds2.extents.y - bounds2.center.y;
