...
public class ExampleCamera : MonoBehaviour
{
	...
	{
		this.Init();
		this.m_camShake = base.GetComponent<XftCameraShakeComp>();
	}
	...
	{
		if (this.m_camShake.enabled)
		{
			return;
		}
		if (Input.mousePosition.x < 120f)
		{
			return;
		}
		...
		{
			this.xDeg += Input.GetAxis("Mouse X") * this.xSpeed * Time.deltaTime;
			this.yDeg -= Input.GetAxis("Mouse Y") * this.ySpeed * Time.deltaTime;
			this.yDeg = ExampleCamera.ClampAngle(this.yDeg, (float)this.yMinLimit, (float)this.yMaxLimit);
	...
	protected XftCameraShakeComp m_camShake;
}
