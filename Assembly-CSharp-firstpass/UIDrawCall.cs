...
public class UIDrawCall : MonoBehaviour
{
	...
	public Texture2D clipMask
	{
		get
		{
			return this.mClipMask;
		}
		set
		{
			this.mClipMask = value;
		}
	}
	...
	private void UpdateMaterials()
	{
		...
		if (flag)
		{
			...
			if (this.mClipping != UIDrawCall.Clipping.None)
			{
				...
				text = text.Replace(" (MaskClip)", string.Empty);
				...
				else if (this.mClipping == UIDrawCall.Clipping.MaskClip)
				{
					shader = Shader.Find(text + " (MaskClip)");
				}
				if (shader == null)
				{
	...
	public void ManualOnWillRenderObject()
	{
		if (this.mReset)
		{
			this.mReset = false;
		...
		if (this.mClippedMat != null)
		{
			...
			if (this.clipping == UIDrawCall.Clipping.MaskClip)
			{
				this.mClippedMat.SetTexture("_ClipMask", this.mClipMask);
			}
			else
			{
