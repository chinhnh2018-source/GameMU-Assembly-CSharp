...
public class UIButtonPlayAnimation : MonoBehaviour
{
	...
	{
		this.mStarted = true;
		if (this.trigger == Trigger.OnPressFalse)
		{
			this.onFinished = delegate(ActiveAnimation anim)
			{
	...
	private void OnEnable()
	{
		...
		if (this.mStarted && this.trigger == Trigger.OnPressFalse && (!this.OnPressFalseTiggered || !this.isFinished))
		{
			this.Play(true);
		}
	}
	...
	private void OnPress(bool isPressed)
	{
		if (base.enabled)
		{
			if ((this.trigger == Trigger.OnPressTrue && isPressed) || (this.trigger == Trigger.OnPressFalse && !isPressed))
			{
				this.Play(true);
			}
			if (this.trigger == Trigger.OnPressFalse)
			{
				if (isPressed)
				{
					this.OnPressFalseTiggered = false;
				}
				else
				{
					this.OnPressFalseTiggered = true;
				}
			}
		}
	}
	...
	private bool OnPressFalseTiggered = true;
