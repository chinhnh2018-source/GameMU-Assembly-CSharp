...
public class UIDraggablePanel : IgnoreTimeScale
{
	...
	public void IsOnTopOrBottom(ref bool top, ref bool bottom)
	{
		top = true;
		bottom = true;
		if (this.mPanel == null)
	...
	public void IsOnLiftOrRight(ref bool top, ref bool bottom)
	{
		top = true;
		bottom = true;
		if (this.mPanel == null)
	...
	public void SetDragAmount(float x, float y, bool updateScrollbars)
	{
		...
		if (!updateScrollbars)
		{
			...
			this.mTrans.localPosition = NGUITools.Round(localPosition);
		}
		clipRange.x = num7;
		clipRange.y = num8;
		this.mPanel.clipRange = NGUITools.Round(clipRange);
		if (updateScrollbars)
		{
	...
	public void MoveRelative(Vector3 relative)
	{
		...
		this.mTrans.localPosition = NGUITools.Round(this.mTrans.localPosition);
		this.mTrans.localPosition += relative;
		Vector4 clipRange = NGUITools.Round(this.mPanel.clipRange);
	...
	public void MoveRelativeEx(Vector3 relative)
	{
		relative = NGUITools.Round(relative);
		this.mTrans.localPosition = NGUITools.Round(this.mTrans.localPosition);
		this.mTrans.localPosition += relative;
	...
	private void LateUpdate()
	{
		...
		if (this.mShouldMove && !this.mPressed)
		{
			...
			if (this.mMomentum.magnitude > 0.0001f)
			{
				...
				this.MoveAbsolute(vector);
				if (this.mMomentum.magnitude > 0.0001f && this.onMomentumIng != null)
				{
					this.onMomentumIng(vector);
				}
				if (this.restrictWithinPanel && this.mPanel.clipping != UIDrawCall.Clipping.None)
				{
					this.RestrictWithinBounds(false);
	...
	public UIDraggablePanel.OnDragIng onDragIng;

	public UIDraggablePanel.OnMomentumIng onMomentumIng;
	...
	public delegate void OnMomentumIng(Vector3 offset);
}
