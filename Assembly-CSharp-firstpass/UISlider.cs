...
public class UISlider : IgnoreTimeScale
{
	...
	private void UpdateDrag()
	{
		...
		Vector3 vector = this.mTrans.localPosition + (this.mCenter - this.mSize * 0.5f);
		Vector3 vector2 = this.mTrans.localPosition - vector;
		Vector3 vector3 = this.mTrans.InverseTransformPoint(ray.GetPoint(num));
		Vector3 vector4 = vector3 + vector2;
		this.Set((this.direction != UISlider.Direction.Horizontal) ? (vector4.y / this.mSize.y) : (vector4.x / this.mSize.x), false);
	}
	...
	private void Set(float input, bool force)
	{
		...
		if (force || sliderValue != sliderValue2)
		{
			...
			{
				this.eventReceiver.SendMessage(this.functionName, sliderValue2, 1);
			}
