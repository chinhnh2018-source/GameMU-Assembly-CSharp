...
public class UIPanel : MonoBehaviour
{
	...
	public UIPanel.DebugInfo debugInfo
	{
		...
		set
		{
			if (this.mDebugInfo != value)
			{
				...
				HideFlags hideFlags = (this.mDebugInfo != UIPanel.DebugInfo.Geometry) ? 61 : 60;
				int i = 0;
	...
	public bool IsVisible(Vector3 worldPos, ref Vector3 localPos)
	{
		if (this.mAlpha < 0.001f)
		{
			return false;
	...
	private void UpdateTransformMatrix()
	{
		if (this.mUpdateTime == 0f || this.mMatrixTime != this.mUpdateTime)
		{
			...
			if (this.mClipping != UIDrawCall.Clipping.None)
			{
				...
				if (vector.x == 0f)
				{
					vector.x = (float)((!(this.mCam == null)) ? this.mCam.pixelWidth : Screen.width);
				}
				if (vector.y == 0f)
				{
					vector.y = (float)((!(this.mCam == null)) ? this.mCam.pixelHeight : Screen.height);
				}
				vector *= 0.5f;
	...
	public void UpdateDrawcalls()
	{
		...
		while (i < size)
		{
			...
			uidrawCall.clipMask = this.mClipMask;
			...
			if (this.mClipping != UIDrawCall.Clipping.None)
			{
				uidrawCall.ManualOnWillRenderObject();
			}
			i++;
		}
	}
