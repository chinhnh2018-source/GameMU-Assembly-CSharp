...
public static class NGUIMath
{
	...
	public static int ColorToIntEx(Color c)
	{
		int num = 0;
		num |= Mathf.RoundToInt(c.r * 255f) << 16;
		num |= Mathf.RoundToInt(c.g * 255f) << 8;
		return num | Mathf.RoundToInt(c.b * 255f);
	}

	public static Color HexToColorEx(uint val)
	{
		float num = 0.003921569f;
		Color black = Color.black;
		black.a = num * (val >> 24 & 255U);
		black.r = num * (val >> 16 & 255U);
		black.g = num * (val >> 8 & 255U);
		black.b = num * (val & 255U);
		if (black.a == 0f)
		{
			black.a = 1f;
		}
	...
	public static Rect ConvertToTexCoords(Rect rect, int width, int height)
	{
		...
		{
			result.xMin = (rect.xMin + 0.5f) / (float)width;
			result.xMax = (rect.xMax - 0.5f) / (float)width;
			result.yMin = 1f - (rect.yMax + 0.5f) / (float)height;
			result.yMax = 1f - (rect.yMin - 0.5f) / (float)height;
		}
		return result;
	}
	...
	public static Rect ConvertToPixels(Rect rect, int width, int height, bool round)
	{
		...
		{
			result.xMin = rect.xMin * (float)width - 0.5f;
			result.xMax = rect.xMax * (float)width + 0.5f;
			result.yMin = (1f - rect.yMax) * (float)height + 0.5f;
			result.yMax = (1f - rect.yMin) * (float)height - 0.5f;
		}
		return result;
	}
	...
	public static Vector3 ApplyHalfPixelOffset(Vector3 pos)
	{
		...
		if (platform == 2 || platform == 5 || platform == 7 || platform == 10)
		{
			pos.x -= 0.5f;
			pos.y += 0.5f;
		}
	...
	public static Vector3[] CalculateWidgetCorners(UIWidget w)
	{
		...
		float num3 = num + relativeSize.x + relativePadding.x + relativePadding.z;
		...
		return new Vector3[]
		{
			...
			cachedTransform.TransformPoint(num, num4, 0f),
			cachedTransform.TransformPoint(num3, num4, 0f),
			cachedTransform.TransformPoint(num3, num2, 0f)
		};
	}
	...
	public static Bounds CalculateAbsoluteWidgetBounds(Transform trans)
	{
		...
		while (i < num)
		{
			...
			Vector3 vector4 = cachedTransform.TransformPoint(new Vector3(num2 - vector3.x, num3 - vector3.y, 0f));
			vector2 = Vector3.Max(vector4, vector2);
			vector = Vector3.Min(vector4, vector);
			vector4 = cachedTransform.TransformPoint(new Vector3(num2 - vector3.x, num3 + vector3.y, 0f));
			vector2 = Vector3.Max(vector4, vector2);
			vector = Vector3.Min(vector4, vector);
			vector4 = cachedTransform.TransformPoint(new Vector3(num2 + vector3.x, num3 - vector3.y, 0f));
			vector2 = Vector3.Max(vector4, vector2);
	...
	public static Bounds CalculateRelativeInnerBounds(Transform root, UISprite sprite)
	{
		if (sprite.type == UISprite.Type.Sliced)
		{
			...
			Vector3 vector2;
			vector2..ctor(num3, num5, 0f);
			vector2 = cachedTransform.TransformPoint(vector2);
			vector2 = worldToLocalMatrix.MultiplyPoint3x4(vector2);
			Bounds result;
			result..ctor(vector2, Vector3.zero);
			vector2..ctor(num3, num6, 0f);
			vector2 = cachedTransform.TransformPoint(vector2);
			vector2 = worldToLocalMatrix.MultiplyPoint3x4(vector2);
			result.Encapsulate(vector2);
			vector2..ctor(num4, num6, 0f);
			vector2 = cachedTransform.TransformPoint(vector2);
			vector2 = worldToLocalMatrix.MultiplyPoint3x4(vector2);
			result.Encapsulate(vector2);
			vector2..ctor(num4, num5, 0f);
			vector2 = cachedTransform.TransformPoint(vector2);
			vector2 = worldToLocalMatrix.MultiplyPoint3x4(vector2);
			result.Encapsulate(vector2);
			return result;
