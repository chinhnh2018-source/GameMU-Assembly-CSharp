...
public class UISprite : UIWidget
{
	...
	{
		Vector2 item;
		item..ctor(this.mOuterUV.xMin, this.mOuterUV.yMin);
		Vector2 item2;
		item2..ctor(this.mOuterUV.xMax, this.mOuterUV.yMax);
	...
	protected void SlicedFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (mainTexture != null)
		{
			...
			Vector2 vector;
			vector..ctor(localScale.x / (float)mainTexture.width, localScale.y / (float)mainTexture.height);
			Vector2 vector2;
			vector2..ctor(num / vector.x, num3 / vector.y);
			Vector2 vector3;
			vector3..ctor(num2 / vector.x, num4 / vector.y);
	...
	public virtual Vector4 drawingDimensions
	{
		get
		{
			Vector2 pivotOffset = NGUITools.GetPivotOffset(base.pivot);
			float num = -pivotOffset.x * base.transform.localScale.x;
			float num2 = -pivotOffset.y * base.transform.localScale.y;
			float num3 = num + base.transform.localScale.x;
	...
	protected void FilledFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		else
		{
					...
					if (this.AdjustRadial(array3, array4, fill, !flag))
					{
						...
						else
						{
							for (int k = 3; k > -1; k--)
							{
								...
								float num15 = Mathf.Lerp(array2[0].x, array2[2].x, array4[k].x);
								float num16 = Mathf.Lerp(array2[0].y, array2[2].y, array4[k].y);
								verts.Add(new Vector3(num11, num12, 0f));
			...
			if (this.fillDirection == UISprite.FillDirection.Radial360)
			{
				...
				for (int l = 0; l < 4; l++)
				{
					...
					for (int m = 0; m < 4; m++)
					{
						int num17 = (!this.mInvert) ? (l * 4) : ((3 - l) * 4);
						float num18 = array5[num17];
						float num19 = array5[num17 + 1];
						float num20 = array5[num17 + 2];
						float num21 = array5[num17 + 3];
						array6[m].x = Mathf.Lerp(num18, num19, array6[m].x);
						array6[m].y = Mathf.Lerp(num20, num21, array6[m].y);
						array7[m].x = Mathf.Lerp(num18, num19, array7[m].x);
						array7[m].y = Mathf.Lerp(num20, num21, array7[m].y);
					}
					...
					if (this.AdjustRadial(array6, array7, fill2, !flag2))
					{
						...
						if (flag2)
						{
							for (int n = 0; n < 4; n++)
							{
								float num22 = Mathf.Lerp(array[0].x, array[2].x, array6[n].x);
								float num23 = Mathf.Lerp(array[0].y, array[2].y, array6[n].y);
								float num24 = Mathf.Lerp(array2[0].x, array2[2].x, array7[n].x);
								float num25 = Mathf.Lerp(array2[0].y, array2[2].y, array7[n].y);
								verts.Add(new Vector3(num22, num23, 0f));
								uvs.Add(new Vector2(num24, num25));
								cols.Add(item);
							}
						}
						else
						{
							for (int num26 = 3; num26 > -1; num26--)
							{
								float num27 = Mathf.Lerp(array[0].x, array[2].x, array6[num26].x);
								float num28 = Mathf.Lerp(array[0].y, array[2].y, array6[num26].y);
								float num29 = Mathf.Lerp(array2[0].x, array2[2].x, array7[num26].x);
								float num30 = Mathf.Lerp(array2[0].y, array2[2].y, array7[num26].y);
								verts.Add(new Vector3(num27, num28, 0f));
		...
		{
			verts.Add(array[num31]);
			uvs.Add(array2[num31]);
			cols.Add(item);
		}
	}
	...
	private bool RadialCut(Vector2[] xy, Vector2[] uv, float fill, bool invert, int corner)
	{
		...
		this.RadialCut(xy, cos, sin, invert, corner);
		this.RadialCut(uv, cos, sin, invert, corner);
		return true;
	}
	...
	protected void TiledFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		Color32 item = (!this.atlas.premultipliedAlpha) ? color : NGUITools.ApplyPMA(color);
		float num3 = 0f;
		Vector4 tiledMargin = this.TiledMargin;
		tiledMargin.x /= vector.x;
		tiledMargin.y /= vector.y;
		tiledMargin.z /= vector.x;
		tiledMargin.w /= vector.y;
		while (num3 < 1f)
		{
			float num4 = 0f;
			vector4.x = vector3.x;
			...
			while (num4 < 1f)
			{
				...
				cols.Add(item);
				cols.Add(item);
				num4 += num + tiledMargin.z;
			}
			num3 += num2 + tiledMargin.w;
		}
	}

	[HideInInspector, SerializeField]
	public Vector4 TiledMargin;
