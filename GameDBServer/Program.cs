...
namespace GameDBServer
{
	public class Program
	{
		...
		private static void ParseInputCmd(string cmd)
		{
			...
			if (Program.CmdDict.TryGetValue(cmd, out cmdCallback) && null != cmdCallback)
			{
				cmdCallback(cmd);
			}
			else
		...
		{
			string value = string.Format("已经连接的登录服务器连接数:{0}", Program.ServerConsole.TotalConnections);
			Console.WriteLine(value);
			value = string.Format("数据库连接池个数: {0}", Program.ServerConsole._DBManger.GetMaxConnsCount());
			Console.WriteLine(value);
			value = string.Format("用户缓存: {0}, 角色缓存: {1}", Program.ServerConsole._DBManger.dbUserMgr.GetUserInfoCount(), Program.ServerConsole._DBManger.DBRoleMgr.GetRoleInfoCount());
			Console.WriteLine(value);
		}
		...
		private void ShowServerTCPInfo(string cmd = null)
		{
			...
			string value = string.Format("总接收字节: {0:0.00} MB", (double)this._TCPManager.MySocketListener.TotalBytesReadSize / 1048576.0);
			Console.WriteLine(value);
			value = string.Format("总发送字节: {0:0.00} MB", (double)this._TCPManager.MySocketListener.TotalBytesWriteSize / 1048576.0);
			Console.WriteLine(value);
			value = string.Format("指令处理平均耗时（毫秒）{0}", (TCPManager.processCmdNum != 0L) ? TimeUtil.TimeMS(TCPManager.processTotalTime / TCPManager.processCmdNum, 2) : 0.0);
			Console.WriteLine(value);
			value = string.Format("指令处理耗时详情", new object[0]);
			Console.WriteLine(value);
			int num = 0;
			lock (TCPManager.cmdMoniter)
			{
				foreach (PorcessCmdMoniter porcessCmdMoniter in TCPManager.cmdMoniter.Values)
				{
					...
					if (flag2)
					{
						if (num++ == 0)
						{
							value = string.Format("{0, -48}{1, 6}{2, 7}{3, 7}{4, 7}{5, 7}", new object[]
							{
								"消息",
								"已处理次数",
								"平均处理时长",
						...
						value = string.Format("{0, -50}{1, 11}{2, 13:0.##}{3, 13:0.##}{4, 13:0.##}{5, 13:0.##}", new object[]
						{
							...
							TimeUtil.TimeMS(porcessCmdMoniter.processMaxTime, 2),
							TimeUtil.TimeMS(porcessCmdMoniter.maxWaitProcessTime, 2)
						});
						Console.WriteLine(value);
					}
					else
					{
						if (num++ == 0)
						{
							value = string.Format("{0, -48}{1, 6}{2, 7}{3, 7}", new object[]
							{
								"消息",
								"已处理次数",
								"平均处理时长",
						...
						value = string.Format("{0, -50}{1, 11}{2, 13:0.##}{3, 13:0.##}", new object[]
						{
							(TCPGameServerCmds)porcessCmdMoniter.cmd,
							porcessCmdMoniter.processNum,
							TimeUtil.TimeMS(porcessCmdMoniter.avgProcessTime(), 2),
					...
					if (flag)
					{
						porcessCmdMoniter.maxWaitProcessTime = 0L;
						porcessCmdMoniter.processMaxTime = 0L;
						porcessCmdMoniter.processNum = 0;
						porcessCmdMoniter.processTotalTime = 0L;
						porcessCmdMoniter.waitProcessTotalTime = 0L;
					}
				}
		...
		private void closingTimer_Tick(object sender, EventArgs e)
		{
			try
			{
				...
				{
					int num = this.ClosingCounter / 200;
					title = string.Format("游戏数据库服务器{0} 关闭中, 倒计时:{1}", GameDBManager.ZoneID, num);
				}
				Console.Title = title;
			}
			catch (Exception e2)
		...
		private void MainDispatcherWorker_DoWork(object sender, EventArgs e)
		{
			...
			for (;;)
			{
				try
				{
					...
					num2 = DateTime.Now.Ticks / 10000L;
					int millisecondsTimeout = (int)Math.Max(1L, (long)num3 - (num2 - num));
		...
		private void UserReturnCheckWorker_DoWork(object sender, DoWorkEventArgs e)
		{
			...
			{
				DataHelper.WriteFormatExceptionLog(e2, "updateFacebook_DoWork", false, false);
			}
		}
		...
		private void updateGiftCode_DoWork(object sender, DoWorkEventArgs e)
		{
			...
			{
				DataHelper.WriteFormatExceptionLog(e2, "updateGiftCode_DoWork", false, false);
			}
		}
		...
		private void ShowInfoTicks(object sender, EventArgs e)
		{
			...
			{
				DataHelper.WriteFormatExceptionLog(e2, "ShowInfoTicks", false, false);
			}
		}
		...
		public void InitServer()
		{
			...
			int num = Math.Max(0, (int)Global.GetSafeAttributeLong(xml, "DBLog", "DBLogEnable"));
			...
			try
			{
				text = Global.GetSafeAttributeStr(xml, "Zone", "URL");
			...
			{
				GetCDBInfoReq getCDBInfoReq = new GetCDBInfoReq();
				getCDBInfoReq.PTID = Global.GetSafeAttributeStr(xml, "Zone", "PTID");
				getCDBInfoReq.ServerID = GameDBManager.ZoneID.ToString();
				getCDBInfoReq.Gamecode = Global.GetSafeAttributeStr(xml, "Zone", "GameCode");
				byte[] array = null;
				byte[] bytes = DataHelper.ObjectToBytes<GetCDBInfoReq>(getCDBInfoReq);
				for (int i = 1; i <= 5; i++)
				{
					array = WebHelper.RequestByPost(text, bytes, 5000, 30000);
					if (array != null)
					{
						break;
					}
				...
				ServerDBInfo serverDBInfo = DataHelper.BytesToObject<ServerDBInfo>(array, 0, array.Length);
				if (serverDBInfo == null)
				{
					throw new Exception(string.Format("请求启动信息 解析失败", new object[0]));
				}
				GameDBManager.serverDBInfo = serverDBInfo;
				...
				string a = string.Format("{0}_game_{1}", getCDBInfoReq.Gamecode, getCDBInfoReq.ServerID);
				if (a != GameDBManager.serverDBInfo.dbName)
				{
			...
			MySQLConnectionString connstr = new MySQLConnectionString(GameDBManager.serverDBInfo.strIP, GameDBManager.serverDBInfo.dbName, GameDBManager.serverDBInfo.userName, GameDBManager.serverDBInfo.uPassword);
			this._DBManger.LoadDatabase(connstr, GameDBManager.serverDBInfo.maxConns, GameDBManager.serverDBInfo.CodePage);
		...
		private void ExecuteBackgroundWorkers(object sender, EventArgs e)
		{
			...
			long num = DateTime.Now.Ticks / 10000L;
			if (num - this.LastReleaseMemoryTicks >= 60000L)
			{
		...
		private void updateMoneyWorker_DoWork(object sender, DoWorkEventArgs e)
		{
			...
			{
				DataHelper.WriteFormatExceptionLog(e2, "updateMoneyWorker_DoWork", false, false);
			}
		}
		...
		private void releaseMemoryWorker_DoWork(object sender, DoWorkEventArgs e)
		{
			try
			{
				...
				if (num >= this.NextReleaseIdleDataTicks)
				{
					this.NextReleaseIdleDataTicks = num + 30000L;
			...
			{
				DataHelper.WriteFormatExceptionLog(e2, "releaseMemoryWorker_DoWork", false, false);
			}
		}
		...
		private void updateLiPinMaWorker_DoWork(object sender, DoWorkEventArgs e)
		{
			...
			{
				DataHelper.WriteFormatExceptionLog(e2, "updateLiPinMaWorker_DoWork", false, false);
			}
		}
		...
		private void updatePreDeleteRoleWorker_DoWork(object sender, DoWorkEventArgs e)
		{
			...
			{
				long num = TimeUtil.NOW();
				if (num >= this.NextPreRemoveRoleTicks)
				{
					this.NextPreRemoveRoleTicks = num + 10000L;
					GameDBManager.PreDelRoleMgr.UpdatePreDeleteRole();
				}
			}
			catch (Exception e2)
			{
				DataHelper.WriteFormatExceptionLog(e2, "updatePreDeleteRoleWorker_DoWork", false, false);
			}
		}
		...
		private void updatePreNamesWorker_DoWork(object sender, DoWorkEventArgs e)
		{
			...
			{
				DataHelper.WriteFormatExceptionLog(e2, "updatePreNamesWorker_DoWork", false, false);
			}
		}
		...
		private void updatePaiHangWorker_DoWork(object sender, DoWorkEventArgs e)
		{
			...
			{
				DataHelper.WriteFormatExceptionLog(e2, "updatePaiHangWorker_DoWork", false, false);
			}
		}
		...
		private void dbGoodsBakTableWorker_DoWork(object sender, EventArgs e)
		{
			...
			{
				long num = TimeUtil.NOW();
				if (num - this.LastdbGoodsBakTableWorkerTicks >= 30000L)
				{
					this.LastdbGoodsBakTableWorkerTicks = num;
		...
		private void updateLastMail_DoWork(object sender, DoWorkEventArgs e)
		{
			...
			{
				DataHelper.WriteFormatExceptionLog(e2, "updateLastMail_DoWork", false, false);
			}
		}
		...
		private void updateGroupMail_DoWork(object sender, DoWorkEventArgs e)
		{
			...
			{
				DataHelper.WriteFormatExceptionLog(e2, "updateGroupMail_DoWork", false, false);
			}
		}
		...
		private void updateTen_DoWork(object sender, DoWorkEventArgs e)
		{
			...
			{
				DataHelper.WriteFormatExceptionLog(e2, "updateTen_DoWork", false, false);
			}
		}
		...
		public static string GetVersionDateTime()
		{
			...
			return dateTime2.ToString("yyyy-MM-dd HH") + string.Format(" {0}", Program.AssemblyFileVersion.FilePrivatePart);
		}
