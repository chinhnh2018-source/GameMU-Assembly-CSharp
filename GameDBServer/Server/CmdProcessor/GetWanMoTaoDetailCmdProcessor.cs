...
namespace GameDBServer.Server.CmdProcessor
{
	public class GetWanMoTaoDetailCmdProcessor : ICmdProcessor
	{
		...
		{
			int nRoleID = DataHelper.BytesToObject<int>(cmdParams, 0, count);
			WanMotaInfo wanMoTaData = WanMoTaManager.getInstance().getWanMoTaData(nRoleID);
			client.sendCmd<WanMotaInfo>(10159, wanMoTaData);
		}
