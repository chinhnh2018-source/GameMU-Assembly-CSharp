...
namespace GameDBServer.Logic.Wing
{
	public class WingPaiHangManager : IManager
	{
		...
		private void initData()
		{
			...
			{
				foreach (WingRankingInfo wingRankingInfo in playerWingDataList)
				{
					this.playerWingDatas.Add(wingRankingInfo.nRoleID, wingRankingInfo);
					this.rankingDatas.Add(wingRankingInfo.getPlayerWingRankingData());
				}
			}
		}
		...
		{
			int num = Math.Max(pageShowNum, WingPaiHangManager.RankingList_PageShowNum);
		...
		public void ModifyWingPaihangData(WingRankingInfo data, bool bIsLogin)
		{
			if (null != data)
			{
				...
				{
					PlayerWingRankingData playerWingRankingData = this.rankingDatas.Find((PlayerWingRankingData paiHang) => paiHang.roleId == data.nRoleID);
					if (null == playerWingRankingData)
					{
					...
					else if (!bIsLogin)
					{
						try
						{
							playerWingRankingData.UpdateData(data);
		...
		{
			WingRankingInfo wingRankingInfo = null;
			lock (this.playerWingDatas)
			{
				...
				wingRankingInfo = this.getWingData(nRoleID);
				if (null != wingRankingInfo)
				{
					this.playerWingDatas.Add(wingRankingInfo.nRoleID, wingRankingInfo);
				}
			}
			if (null != wingRankingInfo)
			{
				this.ModifyWingPaihangData(wingRankingInfo, false);
			}
			return 1;
		}
		...
		{
			WingRankingInfo result = null;
			lock (this.playerWingDatas)
			{
				if (this.playerWingDatas.TryGetValue(nRoleID, out result))
				{
					return result;
				}
			}
		...
		public void onPlayerLogin(int roleId, string strRoleName)
		{
			...
			lock (this.playerWingDatas)
			{
				if (this.playerWingDatas.TryGetValue(roleId, out wingRankingInfo))
				{
					return;
				}
			}
			if (null == wingRankingInfo)
			{
				wingRankingInfo = WingPaiHangDBController.getInstance().getWingDataById(roleId);
				if (null != wingRankingInfo)
				{
					this.ModifyWingPaihangData(wingRankingInfo, true);
					lock (this.playerWingDatas)
					{
						this.playerWingDatas.Add(wingRankingInfo.nRoleID, wingRankingInfo);
					}
		...
		public void onPlayerLogout(int roleId)
		{
			...
			lock (this.playerWingDatas)
			{
				this.playerWingDatas.TryGetValue(roleId, out wingRankingInfo);
				if (null != wingRankingInfo)
				{
		...
		{
			WingRankingInfo wingData = this.getWingData(roleid);
			if (wingData != null)
			{
				wingData.strRoleName = newName;
			}
