...
namespace GameDBServer.Logic
{
	public class FuBenHistManager
	{
		...
		{
			FuBenHistData value = new FuBenHistData
			{
				FuBenID = fuBenID,
				RoleID = roleID,
				RoleName = roleName,
			...
			lock (FuBenHistManager._Mutex)
			{
				FuBenHistManager._FuBenHistDict[fuBenID] = value;
			}
		}
		...
		public static void OnChangeName(int roleId, string oldName, string newName)
		{
			if (!string.IsNullOrEmpty(oldName) && !string.IsNullOrEmpty(newName))
			{
				lock (FuBenHistManager._Mutex)
				{
					...
					foreach (KeyValuePair<int, FuBenHistData> keyValuePair in FuBenHistManager._FuBenHistDict)
					{
						FuBenHistData value = keyValuePair.Value;
						if (value.RoleID == roleId)
						{
							value.RoleName = newName;
						}
					}
				}
